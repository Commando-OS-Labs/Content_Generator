...
const FIXED_WHITE = { r: 1, g: 1, b: 1 };
const FIXED_BLACK = { r: 0, g: 0, b: 0 };
...
let brandLogo = null; // full wordmark (white on transparent)
let circleLogo = null; // circle C icon
var activeBgPattern = "none";
var activeBgDensity = "medium";
var customPatternImage = null;
var activePatternSvgStr = "";
// Blog-specific globals
var activeBlogTitle = "";
var activeBlogAuthor = "";
var activeBlogSections = 3;
const INTER_BOLD = { family: "Inter", style: "Bold" };
const INTER_SEMI = { family: "Inter", style: "Semi Bold" };
...
const BEBAS = { family: "Bebas Neue", style: "Regular" };
const JAKARTA_XBOLD = {
    family: "Plus Jakarta Sans",
    style: "ExtraBold",
};
const DISPLAY_FONTS = [BEBAS, JAKARTA_XBOLD];
// Font fallback system — tracks which fonts loaded successfully
var loadedFonts = new Set();
function fontKey(f) {
    return f.family + "::" + f.style;
}
function resolveFont(target) {
    if (loadedFonts.has(fontKey(target)))
        return target;
    return INTER_BOLD;
}
function getTextEffects(key, fontSize) {
...
function applyTextEffect(node, key) {
    if (key === "clean")
        return;
    var cfg = getTextEffects(key, node.fontSize);
    if (cfg.effects.length > 0)
        node.effects = cfg.effects;
    if (cfg.fills)
        node.fills = cfg.fills;
    if (cfg.strokes) {
        node.strokes = cfg.strokes;
        if (cfg.strokeWeight !== undefined)
...
// ── Accent Decorations ───────────────────────────────────────
function addAccentDots(parent, count, size, x, y, gap, vertical) {
    for (var i = 0; i < count; i++) {
        var dx = vertical ? 0 : i * (size + gap);
        var dy = vertical ? i * (size + gap) : 0;
        addEllipse(parent, "accent-dot-" + i, size, size, x + dx, y + dy, solid(C.red, 0.3 + i * 0.15));
    }
}
function addDiagonalLine(parent, count, length, x, y, gap) {
    for (var i = 0; i < count; i++) {
        var r = addRect(parent, "accent-line-" + i, length, 1, x + i * gap, y + i * gap, solid(C.red, 0.15 + i * 0.08));
        r.rotation = -45;
    }
}
...
function findTextByName(frame, name) {
...
// ── Template Primitives ────────────────────────────────────────
function addText(parent, name, text, font, size, color, x, y, opts) {
    ...
    if (opts && opts.effect)
        applyTextEffect(t, opts.effect);
    return t;
}
function addRect(parent, name, w, h, x, y, fill, radius) {
...
function getDensityValue(density, light, medium, dense) {
    if (density === "light")
        return light;
    if (density === "dense")
        return dense;
...
function generateTopographySvg(w, h, density) {
    ...
    for (var i = 0; i < count; i++) {
        var yBase = Math.round((h / (count + 1)) * (i + 1) + (rand() - 0.5) * h * 0.08);
        var pts = 10;
        var d = "M 0 " + yBase;
        for (var p = 1; p <= pts; p++) {
...
function generateHexGridSvg(w, h, density) {
...
function generateCircuitSvg(w, h, density) {
...
function generateRipplesSvg(w, h, density) {
...
function generateCrossHatchSvg(w, h, density) {
...
function generateDotsSvg(w, h, density) {
...
function generateGridSvg(w, h, density) {
...
function generateDiagonalSvg(w, h, density) {
...
function generateHeroPatternSvg(w, h, tile, density) {
    var tW = tile[0];
    var tH = tile[1];
    var inner = tile[2];
    var color = getPatternColor();
...
function generateGlobeSvg(w, h, density) {
    var color = getPatternColor();
...
function renderSvgPattern(parent, name, svgStr, w, h) {
    try {
...
function renderCustomPattern(parent, w, h) {
    if (!customPatternImage)
        return;
    var dark = C.bg.r < 0.5;
...
function renderIllustrationOverlay(parent, name, svgStr, w, h, density) {
    if (!svgStr)
        return;
    try {
        var node = figma.createNodeFromSvg(svgStr);
        ...
        var dark = C.bg.r < 0.5;
        node.opacity = getDensityValue(density, dark ? 0.12 : 0.06, dark ? 0.2 : 0.1, dark ? 0.3 : 0.16);
        parent.appendChild(node);
    }
    catch (err) {
...
function addPremiumBg(parent, w, h, variant) {
...
function applyBgPattern(parent, w, h, pattern) {
    ...
    switch (pattern) {
        ...
        case "illust-globe-transport":
            renderIllustrationOverlay(parent, "bg-illust-" + pattern, activePatternSvgStr, w, h, density);
            return;
        case "custom":
...
function addEllipse(parent, name, w, h, x, y, fill) {
...
function addCtaButton(parent, fontSize, x, y, padX, padY, text, shadow) {
...
function addBrand(parent, x, y, size, short) {
    ...
    var cText = addText(parent, "brand", "C", INTER_BOLD, Math.round(size * 1.8), FIXED_WHITE, 0, 0);
...
function addSocialIcon(parent, name, svgPath, color, size, x, y) {
    const svgStr = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="' +
        svgPath +
        '"/></svg>';
...
function getBrandFrameConfig(w, h) {
...
function addTopAccentBar(parent, w) {
    addRect(parent, "top-accent", w, 6, 0, 0, solid(C.red));
}
function addBrandFooter(parent, w, h, footerH, logoImage) {
    const y = h - footerH;
    ...
    var footerDarkBg = { r: 0.055, g: 0.055, b: 0.063 };
    var footerDimText = { r: 0.4, g: 0.4, b: 0.4 };
    ...
    if (brandLogo) {
        ...
        addText(parent, "footer-url", "commando360.ai", INTER_REG, urlSize, footerDimText, padX, cy + Math.round(logoH * 0.4));
    }
    else {
        // Fallback: text brand on dark strip
        const brandFontSize = Math.max(8, Math.round(13 * sW));
        addText(parent, "footer-brand", "COMMANDO360.AI", INTER_BOLD, brandFontSize, FIXED_WHITE, padX, cy - Math.round(brandFontSize * 0.6), { letterSpacing: 5 });
    }
    ...
    const totalIconsW = iconSize * socials.length + iconGap * (socials.length - 1);
    ...
    addRect(parent, "footer-div-l", 1, divH, iconsStartX - Math.round(16 * sW), divY, solid(footerDimText, 0.25));
    addRect(parent, "footer-div-r", 1, divH, iconsStartX + totalIconsW + Math.round(16 * sW), divY, solid(footerDimText, 0.25));
    for (let i = 0; i < socials.length; i++) {
        const sx = iconsStartX + i * (iconSize + iconGap);
        addSocialIcon(parent, socials[i].name, socials[i].svg, socials[i].color, iconSize, sx, iconY);
    }
    // ── Right section: QR code + app badges ──
    const qrSize = Math.round(40 * sW);
    const qrX = w - padX - qrSize;
    const qrY = cy - Math.round(qrSize / 2);
    addRect(parent, "qr-code", qrSize, qrSize, qrX, qrY, solid(FIXED_WHITE), Math.round(3 * sW));
    // App store text to the left of QR
    const badgeSize = Math.max(6, Math.round(8 * sW));
    const badgeX = qrX - Math.round(12 * sW);
    var footerGrayText = { r: 0.7, g: 0.7, b: 0.7 };
    const playText = addText(parent, "footer-playstore", "\u25B6 Google Play", INTER_MED, badgeSize, footerGrayText, 0, cy - Math.round(badgeSize * 1.2));
    playText.x = badgeX - playText.width;
    const appText = addText(parent, "footer-appstore", "App Store \u2014 Soon", INTER_REG, Math.max(6, Math.round(7 * sW)), footerDimText, 0, cy + Math.round(badgeSize * 0.3));
    appText.x = badgeX - appText.width;
}
// ══════════════════════════════════════════════════════════════
// BLOG TEMPLATE HELPERS
// ══════════════════════════════════════════════════════════════
function addImagePlaceholder(parent, name, w, h, x, y, caption) {
    ...
    parent.appendChild(r);
    // Centered label
    addText(parent, name + "-label", "IMAGE", INTER_MED, 14, C.bg.r < 0.5 ? { r: 0.35, g: 0.38, b: 0.4 } : { r: 0.55, g: 0.58, b: 0.6 }, x + Math.round(w / 2) - 28, y + Math.round(h / 2) - 8, { opacity: 0.6 });
    var totalH = h;
    if (caption) {
        addText(parent, name + "-caption", caption, INTER_REG, 12, C.bg.r < 0.5
            ? { r: 0.5, g: 0.52, b: 0.55 }
            : { r: 0.45, g: 0.48, b: 0.5 }, x, y + h + 8, { maxWidth: w, opacity: 0.7 });
        totalH += 28;
    }
    return totalH;
}
function addSectionHeading(parent, num, title, y, contentW, contentX) {
    var numStr = num < 10 ? "0" + num : String(num);
    addText(parent, "section_" + num + "_num", numStr, INTER_BOLD, 14, C.red, contentX, y, { opacity: 0.8, letterSpacing: 10 });
    var t = addText(parent, "section_" + num + "_title", title, INTER_BOLD, 28, C.white, contentX, y + 24, { maxWidth: contentW, lineHeight: 130 });
    // Red underline
    addRect(parent, "section-" + num + "-line", 60, 3, contentX, y + 24 + t.height + 12, solid(C.red, 0.6), 2);
    return t.height + 50;
}
function addPullquote(parent, text, attribution, y, contentW, contentX) {
    // Red left bar
    addRect(parent, "pullquote-bar", 4, 80, contentX, y, solid(C.red, 0.8), 2);
    var t = addText(parent, "pullquote", text, INTER_MED, 20, C.white, contentX + 24, y, { maxWidth: contentW - 24, lineHeight: 160, opacity: 0.9 });
    var attrY = y + t.height + 12;
    addText(parent, "pullquote_author", "— " + attribution, INTER_REG, 13, C.gray, contentX + 24, attrY, { opacity: 0.6 });
    // Adjust bar height to match content
    return t.height + 40;
}
function addStatCard(parent, value, label, w, h, x, y) {
    addRect(parent, "stat-card-bg", w, h, x, y, solid(C.bg.r < 0.5
        ? { r: 0.06, g: 0.07, b: 0.08 }
        : { r: 0.95, g: 0.96, b: 0.97 }), 8);
    addText(parent, "stat_" + label.replace(/\s+/g, "_").toLowerCase(), value, INTER_BOLD, 36, C.red, x + Math.round(w / 2) - 30, y + Math.round(h * 0.2), { align: "CENTER", maxWidth: w - 20 });
    addText(parent, "stat_" + label.replace(/\s+/g, "_").toLowerCase() + "_label", label, INTER_REG, 12, C.gray, x + Math.round(w / 2) - 30, y + Math.round(h * 0.65), { align: "CENTER", maxWidth: w - 20, opacity: 0.7 });
}
function addCalloutBox(parent, title, body, accentColor, y, contentW, contentX) {
    var boxH = 120;
    var bg = addRect(parent, "callout-bg", contentW, boxH, contentX, y, solid(C.bg.r < 0.5
        ? { r: 0.05, g: 0.06, b: 0.07 }
        : { r: 0.95, g: 0.96, b: 0.97 }), 8);
    // Left accent bar
    addRect(parent, "callout-accent", 4, boxH, contentX, y, solid(accentColor), 2);
    addText(parent, "callout-title", title, INTER_SEMI, 14, accentColor, contentX + 20, y + 16, { maxWidth: contentW - 40 });
    var bt = addText(parent, "callout-body", body, INTER_REG, 13, C.white, contentX + 20, y + 40, { maxWidth: contentW - 40, lineHeight: 160, opacity: 0.8 });
    var actualH = Math.max(boxH, bt.height + 56);
    if (actualH > boxH)
        bg.resize(contentW, actualH);
    return actualH;
}
// Reusable website nav bar — overlays on hero or sits at top
function addBlogNav(parent, w, y, transparent) {
    var isDark = C.bg.r < 0.5;
    var navH = 64;
    if (transparent) {
        ...
        addRect(parent, "nav-bg", w, navH, 0, y, solid(isDark
            ? { r: 0.027, g: 0.031, b: 0.039 }
            : { r: 0.97, g: 0.97, b: 0.98 }, 0.85));
    }
    addRect(parent, "nav-border", w, 1, 0, y + navH - 1, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.04));
    ...
    if (brandLogo) {
        ...
        addText(parent, "nav-brand-text", "Commando360.ai", INTER_SEMI, 15, transparent ? FIXED_WHITE : C.white, navPad + 36, navCy - 8);
    }
    var navLinkBase = transparent
        ? { r: 0.7, g: 0.7, b: 0.72 }
        : isDark
            ? { r: 0.5, g: 0.5, b: 0.52 }
            : { r: 0.4, g: 0.4, b: 0.42 };
    ...
    for (var ni = navLinks.length - 1; ni >= 0; ni--) {
        var nlColor = navLinks[ni] === "Blog"
            ? transparent
                ? FIXED_WHITE
                : C.white
            : navLinkBase;
        var nt = addText(parent, "nav-" + navLinks[ni].toLowerCase(), navLinks[ni], INTER_MED, 13, nlColor, 0, navCy - 7);
        nt.x = nlX - nt.width;
        nlX -= nt.width + 32;
    }
    return navH;
}
function addBlogHeader(parent, w, title, category, author, readTime, y) {
    var startY = y;
    var isDark = C.bg.r < 0.5;
    var contentX = Math.round((w - 800) / 2);
    // ── Website-style sticky nav bar (glass effect) ──
    y += addBlogNav(parent, w, y);
    var navLinkColor = isDark
        ? { r: 0.5, g: 0.5, b: 0.52 }
        : { r: 0.4, g: 0.4, b: 0.42 };
    // Spacer
    y += 48;
    ...
    addText(parent, "breadcrumbs", crumbStr, INTER_MED, 11, navLinkColor, contentX, y, { letterSpacing: 15, opacity: 0.6 });
    y += 32;
    // ── Category badge ──
    var catW = Math.round(category.length * 8 + 24);
    addRect(parent, "category-badge-bg", catW, 26, contentX, y, solid(C.red, 0.1), 6);
    ...
    addText(parent, "category", category.toUpperCase(), INTER_MED, 11, C.red, contentX + 12, y + 6, { letterSpacing: 10 });
    y += 42;
    // ── Meta line: date | author | read time ──
    var metaStr = "Mar 2026  |  " +
    ...
    var tt = addText(parent, "title", titleText, INTER_BOLD, 42, C.white, contentX, y, { maxWidth: 800, lineHeight: 110 });
    y += tt.height + 28;
...
function addBlogFooter(parent, w, y) {
    var startY = y;
    var isDark = C.bg.r < 0.5;
    var contentX = Math.round((w - 800) / 2);
    var dimColor = isDark
    ...
    addText(parent, "related-label", "RELATED ARTICLES", INTER_MED, 11, dimColor, contentX, y, { letterSpacing: 15, opacity: 0.6 });
    // Horizontal line extending from label
    addRect(parent, "related-line", 560, 1, contentX + 180, y + 6, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.04));
    y += 40;
    ...
    for (var ri = 0; ri < 3; ri++) {
        ...
        // Card background
        addRect(parent, "related_" + (ri + 1) + "_bg", cardW, cardH, cx, y, solid(isDark ? FIXED_WHITE : FIXED_BLACK, isDark ? 0.015 : 0.02), 12);
        // Viewfinder corner brackets
        var cornerSize = 16;
        var cornerStroke = 2;
        var cornerColor = solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.08);
        // Top-left
        addRect(parent, "vf-tl-h-" + ri, cornerSize, cornerStroke, cx, y, cornerColor);
        addRect(parent, "vf-tl-v-" + ri, cornerStroke, cornerSize, cx, y, cornerColor);
        // Top-right
        addRect(parent, "vf-tr-h-" + ri, cornerSize, cornerStroke, cx + cardW - cornerSize, y, cornerColor);
        addRect(parent, "vf-tr-v-" + ri, cornerStroke, cornerSize, cx + cardW - cornerStroke, y, cornerColor);
        // Bottom-left
        addRect(parent, "vf-bl-h-" + ri, cornerSize, cornerStroke, cx, y + cardH - cornerStroke, cornerColor);
        addRect(parent, "vf-bl-v-" + ri, cornerStroke, cornerSize, cx, y + cardH - cornerSize, cornerColor);
        // Bottom-right
        addRect(parent, "vf-br-h-" + ri, cornerSize, cornerStroke, cx + cardW - cornerSize, y + cardH - cornerStroke, cornerColor);
        addRect(parent, "vf-br-v-" + ri, cornerStroke, cornerSize, cx + cardW - cornerStroke, y + cardH - cornerSize, cornerColor);
        // Category badge
        var catText = relatedCategories[ri];
        var catBadgeW = Math.round(catText.length * 7 + 18);
        addRect(parent, "related_" + (ri + 1) + "_cat_bg", catBadgeW, 20, cx + 20, y + 20, solid(C.red, 0.08), 4);
        addText(parent, "related_" + (ri + 1) + "_cat", catText.toUpperCase(), INTER_MED, 9, C.red, cx + 28, y + 25, { letterSpacing: 8 });
        // Title
        addText(parent, "related_" + (ri + 1) + "_title", relatedTitles[ri], INTER_SEMI, 15, C.white, cx + 20, y + 56, { maxWidth: cardW - 40, lineHeight: 135, opacity: 0.85 });
        // Footer divider + meta
        addRect(parent, "related_" + (ri + 1) + "_div", cardW - 40, 1, cx + 20, y + cardH - 36, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.04));
        addText(parent, "related_" + (ri + 1) + "_time", relatedTimes[ri], INTER_REG, 11, faintColor, cx + 20, y + cardH - 24, { letterSpacing: 5 });
    }
    y += cardH;
    // ── Gradient divider ──
    y += 48;
    addBlogGradientDivider(parent, w, Math.round(w * 0.2), Math.round(w * 0.6), y, isDark);
    y += 1;
    // ── Newsletter signup section ──
    y += 48;
    addText(parent, "newsletter-label", "STAY INFORMED", INTER_MED, 11, C.red, Math.round(w / 2) - 50, y, { letterSpacing: 15, opacity: 0.6, align: "CENTER", maxWidth: 100 });
    y += 28;
    addText(parent, "newsletter-heading", "Don't miss a dispatch", INTER_BOLD, 28, C.white, Math.round(w / 2) - 180, y, { maxWidth: 360, align: "CENTER" });
    y += 44;
    addText(parent, "newsletter-sub", "Join security and facility management leaders who get actionable intelligence delivered to their inbox every week.", INTER_REG, 15, dimColor, Math.round(w / 2) - 220, y, { maxWidth: 440, align: "CENTER", lineHeight: 160 });
    y += 60;
    // Email input + Subscribe button
    var formW = 420;
    var formX = Math.round((w - formW) / 2);
    var inputW = formW - 130;
    addRect(parent, "email-input", inputW, 48, formX, y, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.03), 12);
    ...
    addText(parent, "email-placeholder", "your@email.com", INTER_REG, 13, faintColor, formX + 20, y + 16);
    // Subscribe button
    addRect(parent, "subscribe-btn", 120, 48, formX + inputW + 10, y, solid(C.red), 12);
    addText(parent, "subscribe-text", "Subscribe", INTER_SEMI, 13, FIXED_WHITE, formX + inputW + 28, y + 16);
    y += 64;
    addText(parent, "newsletter-disclaimer", "No spam. Unsubscribe anytime. Weekly delivery.", INTER_REG, 11, faintColor, Math.round(w / 2) - 160, y, { maxWidth: 320, align: "CENTER", letterSpacing: 5 });
    y += 28;
    // ── Gradient divider ──
    y += 24;
    addBlogGradientDivider(parent, w, Math.round(w * 0.2), Math.round(w * 0.6), y, isDark);
    ...
    var socialY = y;
    // Left line
    addRect(parent, "social-line-l", 48, 1, Math.round(w / 2) - 130, socialY + 7, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.06));
    addText(parent, "social-linkedin", "LinkedIn", INTER_REG, 12, faintColor, Math.round(w / 2) - 65, socialY, { letterSpacing: 5 });
    addText(parent, "social-sep", "|", INTER_REG, 12, { r: 0.25, g: 0.25, b: 0.27 }, Math.round(w / 2) + 2, socialY);
    addText(parent, "social-x", "@Commando360_AI", INTER_REG, 12, faintColor, Math.round(w / 2) + 18, socialY, { letterSpacing: 5 });
    // Right line
    addRect(parent, "social-line-r", 48, 1, Math.round(w / 2) + 148, socialY + 7, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.06));
    y += 32;
    // ── Minimal site footer (matching website) ──
    y += 24;
    addRect(parent, "site-footer-line", w, 1, 0, y, solid(isDark ? FIXED_WHITE : FIXED_BLACK, 0.04));
    y += 1;
    var footerPadY = y + 28;
    ...
    addText(parent, "site-footer-copy", "\u00A9 2026", INTER_REG, 11, { r: 0.25, g: 0.25, b: 0.27 }, footerPad + (brandLogo ? Math.round(16 * 7.65) + 16 : 40), footerPadY + 2, { letterSpacing: 5 });
    // Right: Privacy · Terms
    var rightX = w - footerPad;
    var termsT = addText(parent, "site-footer-terms", "Terms", INTER_REG, 11, faintColor, 0, footerPadY + 2, { letterSpacing: 5 });
    termsT.x = rightX - termsT.width;
    var privT = addText(parent, "site-footer-privacy", "Privacy", INTER_REG, 11, faintColor, 0, footerPadY + 2, { letterSpacing: 5 });
    privT.x = rightX - termsT.width - 32 - privT.width;
    y = footerPadY + 40;
    return y - startY;
}
// Helper: gradient divider matching website style
function addBlogGradientDivider(parent, w, x, divW, y, isDark) {
...
// ══════════════════════════════════════════════════════════════
// ── 1. Hero Banner ──────────────────────────────────────────
function buildBlogHeroBanner(frame, w, h, sectionCount) {
    var contentW = 800;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    // Full-width hero image
    addImagePlaceholder(frame, "hero-image", w, 600, 0, 0, "Hero image — 1920×1080 recommended");
    ...
    y -= 200;
    var catBadgeY = y;
    addRect(frame, "category-badge-bg", 120, 28, contentX, catBadgeY, solid(C.red, 0.15), 14);
    addText(frame, "category", "Technology", INTER_SEMI, 11, FIXED_WHITE, contentX + 20, catBadgeY + 7);
    y = catBadgeY + 44;
    var titleStr = activeBlogTitle ||
        "AI-Powered Command Centers: The New Nerve Center of Security";
    var tt = addText(frame, "title", titleStr, INTER_BOLD, 42, FIXED_WHITE, contentX, y, { maxWidth: contentW, lineHeight: 120 });
    y += tt.height + 16;
    var metaStr = (activeBlogAuthor || "Commando360 Team") + "  ·  7 min read  ·  Mar 2026";
    ...
    for (var si = 0; si < 4; si++) {
        addStatCard(frame, stats[si][0], stats[si][1], statW, 90, contentX + si * (statW + 20), y);
    }
    y += 130;
    // Body sections
    for (var s = 0; s < sectionCount; s++) {
        ...
        y += addSectionHeading(frame, s + 1, titles[s % titles.length], y, contentW, contentX);
        y += 16;
        var bodyText = BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length];
        var bt = addText(frame, "section_" + (s + 1) + "_body", bodyText, INTER_REG, 16, C.white, contentX, y, { maxWidth: contentW, lineHeight: 175, opacity: 0.85 });
        y += bt.height + 24;
        // Breakout image after section 1
        if (s === 0) {
            y += addImagePlaceholder(frame, "breakout-image", w, 400, 0, y, "Full-width breakout image");
            y += 20;
        }
        // Callout after section 2
        if (s === 1) {
            y += addCalloutBox(frame, "Key Insight", "Organizations using AI-powered command centers report 87% faster incident response times and 45% reduction in security breaches.", C.red, y, contentW, contentX);
            y += 20;
        }
    }
    // Footer
...
// ── 2. Editorial ────────────────────────────────────────────
function buildBlogEditorial(frame, w, h, sectionCount) {
    var contentW = 720;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    // Header
    y += addBlogHeader(frame, w, "", "Industry Insights", activeBlogAuthor || "Commando360 Team", "8 min read", 0);
    // Hero image
    y += 20;
    y += addImagePlaceholder(frame, "hero-image", contentW, 400, contentX, y, "Featured image — 960×540 recommended");
    y += 40;
    // Excerpt (italic)
    var excerptText = "How the security industry is transforming from reactive guarding to proactive, technology-enabled security operations.";
    var et = addText(frame, "excerpt", excerptText, INTER_MED, 18, C.white, contentX, y, { maxWidth: contentW, lineHeight: 170, opacity: 0.7 });
    y += et.height + 30;
    // Divider
    addRect(frame, "excerpt-divider", contentW, 1, contentX, y, solid(C.bg.r < 0.5
        ? { r: 0.15, g: 0.16, b: 0.18 }
        : { r: 0.85, g: 0.86, b: 0.87 }));
    y += 30;
    // Drop cap first paragraph
    var firstBody = BLOG_PLACEHOLDER_BODY;
    addText(frame, "drop-cap", firstBody.charAt(0), INTER_BOLD, 72, C.red, contentX, y - 4);
    var restText = firstBody.substring(1);
    var ft = addText(frame, "section_1_body", restText, INTER_REG, 16, C.white, contentX + 52, y, { maxWidth: contentW - 52, lineHeight: 180, opacity: 0.85 });
    y += ft.height + 40;
    // Remaining sections
    for (var s = 1; s < sectionCount; s++) {
        ...
        var tt = addText(frame, "section_" + (s + 1) + "_title", titles[s % titles.length], INTER_BOLD, 26, C.white, contentX, y, { maxWidth: contentW, lineHeight: 130 });
        y += tt.height + 16;
        var bt = addText(frame, "section_" + (s + 1) + "_body", BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length], INTER_REG, 16, C.white, contentX, y, { maxWidth: contentW, lineHeight: 180, opacity: 0.85 });
        y += bt.height + 30;
        // Pullquote after section 2
        if (s === 1) {
            y += addPullquote(frame, "The future belongs to security companies that can blend human expertise with intelligent technology.", "Industry Analyst", y, contentW, contentX);
            y += 30;
        }
    }
    // Author bio
    y += 30;
    addRect(frame, "author-bio-bg", contentW, 100, contentX, y, solid(C.bg.r < 0.5
        ? { r: 0.06, g: 0.07, b: 0.08 }
        : { r: 0.94, g: 0.95, b: 0.96 }), 8);
    // Avatar circle
    ...
    addText(frame, "author", activeBlogAuthor || "Commando360 Team", INTER_SEMI, 15, C.white, contentX + 90, y + 24);
    addText(frame, "author-role", "Security Operations Platform", INTER_REG, 12, C.gray, contentX + 90, y + 48, { opacity: 0.6 });
    y += 130;
    // Footer
    y += addBlogFooter(frame, w, y);
    return y;
...
// ── 3. Case Study ───────────────────────────────────────────
function buildBlogCaseStudy(frame, w, h, sectionCount) {
    var contentW = 820;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    ...
    addText(frame, "badge", "Case Study", INTER_SEMI, 11, C.red, contentX + 16, y + 6);
    y += 44;
    var titleStr = activeBlogTitle || "How SecureGuard Reduced Incident Response Time by 87%";
    var tt = addText(frame, "title", titleStr, INTER_BOLD, 36, C.white, contentX, y, { maxWidth: contentW, lineHeight: 125 });
    y += tt.height + 16;
    addText(frame, "meta", (activeBlogAuthor || "Commando360 Team") + "  ·  6 min read", INTER_REG, 13, C.gray, contentX, y, { opacity: 0.5 });
    y = 360;
    ...
    for (var ri = 0; ri < 4; ri++) {
        addStatCard(frame, results[ri][0], results[ri][1], snapW, 85, contentX + ri * (snapW + 10), y);
    }
    y += 120;
    // Featured image
    y += addImagePlaceholder(frame, "featured-image", contentW, 400, contentX, y, "Client site or operations center");
    y += 30;
    ...
    for (var s = 0; s < sectionCount; s++) {
        addText(frame, "section_label_" + (s + 1), csLabels[s % csLabels.length].toUpperCase(), INTER_SEMI, 11, C.red, contentX, y, { letterSpacing: 15, opacity: 0.8 });
        y += 24;
        var st = addText(frame, "section_" + (s + 1) + "_title", csLabels[s % csLabels.length], INTER_BOLD, 24, C.white, contentX, y, { maxWidth: contentW });
        y += st.height + 12;
        var bt = addText(frame, "section_" + (s + 1) + "_body", BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length], INTER_REG, 16, C.white, contentX, y, { maxWidth: contentW, lineHeight: 175, opacity: 0.85 });
        y += bt.height + 30;
        // Before/After cards after "The Challenge"
        if (s === 1) {
            var halfW = Math.round((contentW - 20) / 2);
            addRect(frame, "before-card", halfW, 160, contentX, y, solid(C.bg.r < 0.5
                ? { r: 0.06, g: 0.07, b: 0.08 }
                : { r: 0.94, g: 0.95, b: 0.96 }), 8);
            addRect(frame, "before-accent", halfW, 3, contentX, y, solid(C.gray, 0.3));
            addText(frame, "before-label", "BEFORE", INTER_SEMI, 12, C.gray, contentX + 16, y + 16, { opacity: 0.5 });
            addText(frame, "before-text", "Manual paper-based incident reports\nAverage 45-minute response time\nNo real-time visibility across sites", INTER_REG, 13, C.white, contentX + 16, y + 44, { maxWidth: halfW - 32, lineHeight: 170, opacity: 0.7 });
            var afterX = contentX + halfW + 20;
            addRect(frame, "after-card", halfW, 160, afterX, y, solid(C.bg.r < 0.5
                ? { r: 0.06, g: 0.07, b: 0.08 }
                : { r: 0.94, g: 0.95, b: 0.96 }), 8);
            addRect(frame, "after-accent", halfW, 3, afterX, y, solid(C.red, 0.6));
            addText(frame, "after-label", "AFTER", INTER_SEMI, 12, C.red, afterX + 16, y + 16);
            addText(frame, "after-text", "Digital real-time incident reporting\nAverage 6-minute response time\nLive dashboard for all 240+ sites", INTER_REG, 13, C.white, afterX + 16, y + 44, { maxWidth: halfW - 32, lineHeight: 170, opacity: 0.7 });
            y += 190;
        }
        // Client quote
        if (s === 2) {
            y += addPullquote(frame, "Commando360 transformed how we manage security across all our locations. The visibility alone was worth the investment.", "VP of Security, SecureGuard Inc.", y, contentW, contentX);
            y += 20;
        }
    }
    y += addBlogFooter(frame, w, y);
...
// ── 4. Changelog ────────────────────────────────────────────
function buildBlogChangelog(frame, w, h, sectionCount) {
    var contentW = 800;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    // Header
    y += addBlogHeader(frame, w, "", "Product Updates", activeBlogAuthor || "Commando360 Team", "5 min read", 0);
    // Version tag
    addRect(frame, "version-bg", 80, 28, contentX, y, solid(C.red, 0.12), 14);
    addText(frame, "version", "v3.2.0", INTER_SEMI, 12, C.red, contentX + 14, y + 6);
    y += 48;
    // Changelog title override
    var titleStr = activeBlogTitle || "March 2026 Product Updates";
    var tt = addText(frame, "changelog-title", titleStr, INTER_BOLD, 32, C.white, contentX, y, { maxWidth: contentW });
    y += tt.height + 12;
    var es = addText(frame, "excerpt", "New features, improvements, and bug fixes for March 2026.", INTER_REG, 16, C.gray, contentX, y, { maxWidth: contentW, opacity: 0.6 });
    y += es.height + 40;
    // Changelog entries
    var badgeColors = {
    ...
    for (var ei = 0; ei < entryCount; ei++) {
        ...
        // Date
        addText(frame, "entry_" + ei + "_date", entry.date, INTER_REG, 12, C.gray, contentX, y + 2, { opacity: 0.5 });
        // Badge
        var badgeColor = badgeColors[entry.badge] || C.gray;
        addRect(frame, "entry_" + ei + "_badge_bg", 70, 22, contentX + 60, y, solid(badgeColor, 0.15), 11);
        addText(frame, "entry_" + ei + "_badge", entry.badge, INTER_SEMI, 10, badgeColor, contentX + 74, y + 4);
        // Title & body
        var entryT = addText(frame, "entry_" + ei + "_title", entry.title, INTER_SEMI, 17, C.white, contentX + 150, y, { maxWidth: contentW - 150 });
        y += entryT.height + 8;
        var entryB = addText(frame, "entry_" + ei + "_body", entry.body, INTER_REG, 14, C.white, contentX + 150, y, { maxWidth: contentW - 150, lineHeight: 165, opacity: 0.75 });
        y += entryB.height + 12;
        // Screenshot placeholder for major entries
        if (ei < 2) {
            y += addImagePlaceholder(frame, "entry_" + ei + "_screenshot", contentW - 150, 200, contentX + 150, y, "Feature screenshot");
            y += 8;
        }
        // Divider
        addRect(frame, "entry_" + ei + "_divider", contentW, 1, contentX, y, solid(C.bg.r < 0.5
...
// ── 5. Comparison ───────────────────────────────────────────
function buildBlogComparison(frame, w, h, sectionCount) {
    var contentW = 820;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    y += addBlogHeader(frame, w, "", "Comparison", activeBlogAuthor || "Commando360 Team", "10 min read", 0);
    // Excerpt
    var excerptStr = "A comprehensive feature-by-feature comparison to help you make the right choice for your security operations.";
    var et = addText(frame, "excerpt", excerptStr, INTER_REG, 16, C.gray, contentX, y, { maxWidth: contentW, lineHeight: 170, opacity: 0.6 });
    y += et.height + 40;
    // Comparison table
    var colW = Math.round((contentW - 200) / 2);
    // Table header
    addRect(frame, "table-header-bg", contentW, 48, contentX, y, solid(C.bg.r < 0.5
        ? { r: 0.08, g: 0.09, b: 0.1 }
        : { r: 0.92, g: 0.93, b: 0.94 }), 8);
    addText(frame, "table-header-feature", "Feature", INTER_SEMI, 13, C.gray, contentX + 16, y + 15);
    addText(frame, "table-header-c360", "Commando360", INTER_SEMI, 13, C.red, contentX + 216, y + 15);
    addText(frame, "table-header-trad", "Traditional", INTER_SEMI, 13, C.gray, contentX + 216 + colW + 16, y + 15, { opacity: 0.6 });
    y += 52;
    // Table rows
    var rows = [
    ...
    for (var ri = 0; ri < rowCount; ri++) {
        ...
        addText(frame, "row_" + ri + "_feature", rows[ri][0], INTER_MED, 13, C.white, contentX + 16, y + 15, { maxWidth: 184, opacity: 0.9 });
        addText(frame, "row_" + ri + "_c360", rows[ri][1], INTER_REG, 13, C.white, contentX + 216, y + 15, { maxWidth: colW - 16, opacity: 0.85 });
        addText(frame, "row_" + ri + "_trad", rows[ri][2], INTER_REG, 13, C.gray, contentX + 216 + colW + 16, y + 15, { maxWidth: colW - 16, opacity: 0.5 });
        y += 50;
    }
    y += 30;
    // Body sections
    for (var s = 0; s < Math.min(sectionCount, 3); s++) {
        y += addSectionHeading(frame, s + 1, [
            "Deep Dive: Operational Efficiency",
            "The Total Cost of Ownership",
            "Implementation & Onboarding",
        ][s % 3], y, contentW, contentX);
        y += 16;
        var bt = addText(frame, "section_" + (s + 1) + "_body", BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length], INTER_REG, 16, C.white, contentX, y, { maxWidth: contentW, lineHeight: 175, opacity: 0.85 });
        y += bt.height + 30;
    }
...
// ── 6. Data Report ──────────────────────────────────────────
function buildBlogDataReport(frame, w, h, sectionCount) {
    var sidebarW = 220;
    var mainW = 820;
    ...
    var y = 0;
    // Nav bar (full width)
    addRect(frame, "nav-bg", w, 64, 0, 0, solid(C.bg.r < 0.5
    ...
    for (var ti = 0; ti < Math.min(tocTitles.length, sectionCount + 1); ti++) {
        ...
        addText(frame, "toc_" + ti, numStr + "  " + tocTitles[ti], INTER_REG, 13, ti === 0 ? C.red : C.gray, 40, tocY, { maxWidth: sidebarW - 20, opacity: ti === 0 ? 1 : 0.6 });
        tocY += 32;
    }
    // Main content
    y += 40;
    addRect(frame, "report-badge-bg", 80, 26, mainX, y, solid(C.red, 0.12), 13);
    addText(frame, "category", "Report", INTER_SEMI, 11, C.red, mainX + 16, y + 6);
    y += 44;
    var titleStr = activeBlogTitle || "2026 Security Operations Benchmark Report";
    var tt = addText(frame, "title", titleStr, INTER_BOLD, 36, C.white, mainX, y, { maxWidth: contentW, lineHeight: 120 });
    y += tt.height + 12;
    addText(frame, "meta", (activeBlogAuthor || "Commando360 Team") + "  ·  12 min read", INTER_REG, 13, C.gray, mainX, y, { opacity: 0.5 });
    y += 40;
    // Hero image
    y += addImagePlaceholder(frame, "hero-image", contentW, 320, mainX, y, "Report cover or key visual");
    y += 40;
    // Stat cards row
    ...
    for (var si = 0; si < 4; si++) {
        addStatCard(frame, reportStats[si][0], reportStats[si][1], scW, 85, mainX + si * (scW + 10), y);
    }
    y += 120;
    // Numbered sections
    for (var s = 0; s < sectionCount; s++) {
        y += addSectionHeading(frame, s + 1, tocTitles[(s + 1) % tocTitles.length], y, contentW, mainX);
        y += 16;
        var bt = addText(frame, "section_" + (s + 1) + "_body", BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length], INTER_REG, 16, C.white, mainX, y, { maxWidth: contentW, lineHeight: 175, opacity: 0.85 });
        y += bt.height + 24;
        // Chart placeholder after first section
        if (s === 0) {
            y += addImagePlaceholder(frame, "chart-1", contentW, 280, mainX, y, "Chart or data visualization");
            addText(frame, "chart-1-cite", "Source: Commando360 Security Operations Survey, 2026 (n=1,247)", INTER_REG, 11, C.gray, mainX, y - 8, { opacity: 0.4 });
            y += 20;
        }
        // Callout after section 2
        if (s === 1) {
            y += addCalloutBox(frame, "Key Finding", "Organizations that adopted integrated security platforms saw a 3.2x improvement in incident detection rates compared to those using fragmented point solutions.", { r: 0.13, g: 0.77, b: 0.37 }, y, contentW, mainX);
            y += 20;
        }
    }
    y += addBlogFooter(frame, w, y);
...
// ── 7. Immersive ────────────────────────────────────────────
function buildBlogImmersive(frame, w, h, sectionCount) {
    var contentW = 700;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    // Full-bleed hero
    addImagePlaceholder(frame, "hero-image", w, 700, 0, 0, "Dramatic hero image — 1920×1080");
    ...
    addText(frame, "kicker", "VISUAL STORY", INTER_SEMI, 12, FIXED_WHITE, Math.round(w / 2) - 50, 480, { letterSpacing: 20, opacity: 0.7 });
    var titleStr = activeBlogTitle || "A Night in the Life of Security Operations";
    var tt = addText(frame, "title", titleStr, INTER_BOLD, 48, FIXED_WHITE, Math.round(w / 2) - 350, 510, { maxWidth: 700, align: "CENTER", lineHeight: 115 });
    // Red divider under title
    addRect(frame, "hero-divider", 60, 3, Math.round(w / 2) - 30, 510 + tt.height + 16, solid(C.red));
    y = 720;
    // Intro paragraph (narrow)
    y += 20;
    var introT = addText(frame, "intro", "As the sun sets and the city transitions from day to night, a different kind of workforce takes over. These are the stories of the professionals who keep our world secure while most of us sleep.", INTER_REG, 18, C.white, contentX, y, { maxWidth: contentW, lineHeight: 185, opacity: 0.9 });
    y += introT.height + 60;
    // Alternating sections with full-bleed images
    for (var s = 0; s < sectionCount; s++) {
        // Full-bleed image
        y += addImagePlaceholder(frame, "fullbleed_" + (s + 1), w, 500, 0, y, "Full-width story image " + (s + 1));
        y += 40;
        // Text section
        if (s < 3) {
            var titles = [
            ...
            var st = addText(frame, "section_" + (s + 1) + "_title", titles[s % titles.length], INTER_BOLD, 28, C.white, contentX, y, { maxWidth: contentW, lineHeight: 130 });
            y += st.height + 16;
        }
        var bt = addText(frame, "section_" + (s + 1) + "_body", BLOG_PLACEHOLDER_BODIES[s % BLOG_PLACEHOLDER_BODIES.length], INTER_REG, 17, C.white, contentX, y, { maxWidth: contentW, lineHeight: 185, opacity: 0.85 });
        y += bt.height + 30;
        // Pullquote in middle
        if (s === 1) {
            y += addPullquote(frame, "There is a quiet dignity in protecting people while they sleep. Every patrol, every checkpoint, every alert — it all matters.", "Senior Guard, Night Watch Division", y, contentW, contentX);
            y += 30;
        }
    }
    // Gallery
    y += 20;
    addText(frame, "gallery-label", "Gallery", INTER_SEMI, 18, C.white, contentX, y);
    y += 40;
    var galleryColW = Math.round((contentW - 20) / 2);
    for (var gi = 0; gi < 4; gi++) {
        var gx = contentX + (gi % 2) * (galleryColW + 20);
        var gy = y + Math.floor(gi / 2) * 230;
        addImagePlaceholder(frame, "gallery_" + (gi + 1), galleryColW, 210, gx, gy, "Gallery image " + (gi + 1));
    }
    y += 480;
    // Closing visual
    addImagePlaceholder(frame, "closing-image", w, 400, 0, y, "Closing dramatic image");
    ...
    addText(frame, "closing-text", "Every night. Every site. Every guard. Security never sleeps.", INTER_BOLD, 28, FIXED_WHITE, Math.round(w / 2) - 250, y + 170, { maxWidth: 500, align: "CENTER", lineHeight: 140 });
    y += 420;
    y += addBlogFooter(frame, w, y);
    return y;
}
// ── 8. Tutorial ─────────────────────────────────────────────
function buildBlogTutorial(frame, w, h, sectionCount) {
    var contentW = 780;
    var contentX = Math.round((w - contentW) / 2);
    var y = 0;
    y += addBlogHeader(frame, w, "", "Tutorial", activeBlogAuthor || "Commando360 Team", "12 min read", 0);
    // Hero image
    y += addImagePlaceholder(frame, "hero-image", contentW, 380, contentX, y, "Tutorial cover image — 1920×1080");
    y += 40;
    // Info boxes (2 columns)
    var boxW = Math.round((contentW - 20) / 2);
    var boxH = 160;
    // What You'll Learn box
    addRect(frame, "learn-box-bg", boxW, boxH, contentX, y, solid(C.bg.r < 0.5
        ? { r: 0.06, g: 0.04, b: 0.1 }
        : { r: 0.95, g: 0.94, b: 0.98 }), 10);
    addRect(frame, "learn-box-accent", 4, boxH, contentX, y, solid({ r: 0.55, g: 0.36, b: 0.96 }), 2);
    addText(frame, "learn-title", "What You'll Learn", INTER_SEMI, 14, { r: 0.55, g: 0.36, b: 0.96 }, contentX + 20, y + 16);
    addText(frame, "learn-items", "• Set up real-time GPS guard tracking\n• Configure geofence boundaries for sites\n• Enable automated deviation alerts\n• Generate patrol compliance reports", INTER_REG, 13, C.white, contentX + 20, y + 44, { maxWidth: boxW - 40, lineHeight: 170, opacity: 0.8 });
    // Prerequisites box
    var preX = contentX + boxW + 20;
    addRect(frame, "prereq-box-bg", boxW, boxH, preX, y, solid(C.bg.r < 0.5
        ? { r: 0.1, g: 0.04, b: 0.04 }
        : { r: 0.98, g: 0.95, b: 0.95 }), 10);
    addRect(frame, "prereq-box-accent", 4, boxH, preX, y, solid(C.red), 2);
    addText(frame, "prereq-title", "Prerequisites", INTER_SEMI, 14, C.red, preX + 20, y + 16);
    addText(frame, "prereq-items", "• Commando360 admin account\n• At least one active site configured\n• Mobile app installed on guard devices\n• GPS permissions enabled", INTER_REG, 13, C.white, preX + 20, y + 44, { maxWidth: boxW - 40, lineHeight: 170, opacity: 0.8 });
    y += boxH + 40;
    // Steps
    var stepTitles = [
    ...
    for (var s = 0; s < Math.min(sectionCount + 2, stepTitles.length); s++) {
        ...
        addText(frame, "step_" + (s + 1) + "_num", String(s + 1), INTER_BOLD, 18, FIXED_WHITE, contentX + (s + 1 < 10 ? 14 : 9), y + 9);
        // Step title
        var st = addText(frame, "section_" + (s + 1) + "_title", stepTitles[s], INTER_SEMI, 20, C.white, contentX + 56, y + 8, { maxWidth: contentW - 56 });
        y += Math.max(st.height + 16, 48);
        // Step body
        var bt = addText(frame, "section_" + (s + 1) + "_body", stepBodies[s], INTER_REG, 15, C.white, contentX + 56, y, { maxWidth: contentW - 56, lineHeight: 175, opacity: 0.85 });
        y += bt.height + 16;
        // Screenshot for each step
        y += addImagePlaceholder(frame, "step_" + (s + 1) + "_image", contentW - 56, 220, contentX + 56, y, "Step " + (s + 1) + " screenshot");
        y += 8;
        // Tip callout after step 2
        if (s === 1) {
            y += addCalloutBox(frame, "Pro Tip", "Start with your highest-traffic site first. The data you collect will help you fine-tune settings before rolling out to all locations.", { r: 0.23, g: 0.51, b: 0.96 }, y, contentW, contentX);
            y += 16;
        }
        // Connector line between steps
        if (s < stepTitles.length - 1 &&
            s < Math.min(sectionCount + 2, stepTitles.length) - 1) {
            addRect(frame, "step-connector-" + s, 2, 24, contentX + 19, y, solid(C.red, 0.3));
            y += 28;
        }
    }
    // Summary
    y += 20;
    y += addCalloutBox(frame, "Summary", "You've configured GPS tracking, geofences, checkpoints, and alerts. Your guard tour system is now fully operational. Monitor the compliance dashboard regularly and adjust thresholds as needed.", C.red, y, contentW, contentX);
    y += addBlogFooter(frame, w, y);
    return y;
}
...
function buildVerticalSplit(frame, w, h) {
    ...
    addText(frame, "image-label", "DROP\nIMAGE\nHERE", INTER_MED, Math.round(w * 0.02), C.dim, Math.round(splitX * 0.3), Math.round(h * 0.4), { align: "CENTER" });
    ...
    addText(frame, "headline", "Your Headline\nGoes Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.038), C.white, splitX + pad, Math.round(h * 0.22), { maxWidth: rightW - pad * 2, lineHeight: 115, effect: "lifted" });
    addText(frame, "description", "Benefit-focused description text goes here.", INTER_REG, Math.round(w * 0.017), C.gray, splitX + pad, Math.round(h * 0.55), { maxWidth: rightW - pad * 2, lineHeight: 155 });
    addCtaButton(frame, Math.round(w * 0.017), splitX + pad, Math.round(h * 0.75), Math.round(w * 0.022), Math.round(w * 0.012), undefined, true);
...
function buildTypographyHero(frame, w, h) {
    const pad = Math.round(w * 0.07);
    addPremiumBg(frame, w, h, 1);
    // Massive headline — fills the frame
    addText(frame, "headline", "Your\nHeadline\nHere", resolveFont(BEBAS), Math.round(w * 0.1), C.white, pad, Math.round(h * 0.1), {
        maxWidth: Math.round(w * 0.86),
        lineHeight: 95,
        effect: "glow",
    });
    // Thin red accent line as divider
    addRect(frame, "divider", Math.round(w * 0.15), 2, pad, Math.round(h * 0.65), solid(C.red));
    // Small description below divider
    addText(frame, "description", "Benefit-focused description text goes here.", INTER_REG, Math.round(w * 0.018), C.gray, pad, Math.round(h * 0.7), { maxWidth: Math.round(w * 0.55), lineHeight: 155 });
    // CTA at bottom-left
    addCtaButton(frame, Math.round(w * 0.017), pad, Math.round(h * 0.85), Math.round(w * 0.022), Math.round(w * 0.012), undefined, true);
    // Diagonal line accents
    addDiagonalLine(frame, 4, Math.round(w * 0.06), w - pad - Math.round(w * 0.12), Math.round(h * 0.12), 8);
    // Brand logo at bottom-right
    addBrand(frame, w - pad - Math.round(w * 0.15), Math.round(h * 0.88), Math.round(w * 0.011), true);
}
// ── Layout 3: Circle Crop (fashion / personal brand style) ──
function buildCircleCrop(frame, w, h) {
    ...
    addText(frame, "headline", "Your Headline", resolveFont(BEBAS), Math.round(w * 0.035), C.white, Math.round(w * 0.5), Math.round(h * 0.06), { maxWidth: Math.round(w * 0.42), lineHeight: 110, align: "RIGHT" });
    // Red accent ring behind circle
    addRect(frame, "circle-ring", circleD + 6, circleD + 6, circleX - 3, circleY - 3, solid(C.red, 0.5), circleR + 3);
    // Circle image area
    addRect(frame, "image-area", circleD, circleD, circleX, circleY, solid(C.card), circleR);
    addText(frame, "image-label", "DROP\nIMAGE", INTER_MED, Math.round(w * 0.018), C.dim, cx - Math.round(w * 0.04), circleY + circleR - Math.round(w * 0.03), { align: "CENTER" });
    // Description below circle
    const descY = circleY + circleD + Math.round(h * 0.04);
    addText(frame, "description", "Benefit-focused description text goes here.", INTER_REG, Math.round(w * 0.018), C.gray, pad, descY, { maxWidth: Math.round(w * 0.6), lineHeight: 150 });
    // CTA at bottom-right
    addCtaButton(frame, Math.round(w * 0.017), w - pad - Math.round(w * 0.22), descY, Math.round(w * 0.022), Math.round(w * 0.012));
}
// ── Layout 4: Horizontal Split (Apple product launch style) ──
function buildHorizontalSplit(frame, w, h) {
    ...
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.018), C.dim, Math.round(w * 0.35), Math.round(splitY * 0.45), { align: "CENTER" });
    ...
    addText(frame, "headline", "Your Headline Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.038), C.white, pad, textY, { maxWidth: Math.round(w * 0.75), lineHeight: 115, effect: "lifted" });
    addText(frame, "description", "Benefit-focused description text.", INTER_REG, Math.round(w * 0.017), C.gray, pad, textY + Math.round(bottomH * 0.38), { maxWidth: Math.round(w * 0.55), lineHeight: 150 });
    // CTA at bottom-left
    addCtaButton(frame, Math.round(w * 0.017), pad, h - Math.round(bottomH * 0.2), Math.round(w * 0.022), Math.round(w * 0.012), undefined, true);
    // Brand at bottom-right
    addBrand(frame, w - pad - Math.round(w * 0.13), h - Math.round(bottomH * 0.25), Math.round(w * 0.011), true);
}
// ── Layout 5: Frame-in-Frame (Premium gallery / luxury style) ──
function buildFrameInFrame(frame, w, h) {
    ...
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.016), C.dim, inset + Math.round(innerW * 0.32), inset + Math.round(imgH * 0.45), { align: "CENTER" });
    // Text area below image
    const textY = inset + imgH + Math.round(innerH * 0.06);
    addText(frame, "headline", "Your Headline Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.035), C.white, inset + pad, textY, { maxWidth: innerW - pad * 2, lineHeight: 115, effect: "outline" });
    addText(frame, "description", "Benefit-focused description text goes here.", INTER_REG, Math.round(w * 0.016), C.gray, inset + pad, textY + Math.round(innerH * 0.18), { maxWidth: Math.round(innerW * 0.6), lineHeight: 150 });
    // CTA at bottom-right of inner frame
    addCtaButton(frame, Math.round(w * 0.016), inset + innerW - pad - Math.round(w * 0.18), textY + Math.round(innerH * 0.18), Math.round(w * 0.02), Math.round(w * 0.01));
    // Brand logo at bottom-left of inner frame
    addBrand(frame, inset + pad, h - inset - Math.round(innerH * 0.06), Math.round(w * 0.011), true);
}
// ══════════════════════════════════════════════════════════════
// CAROUSEL LAYOUTS (3 slide types at 1080×1080)
// ══════════════════════════════════════════════════════════════
function buildCarouselCover(frame, w, h) {
    const cx = Math.round(w / 2);
    addPremiumBg(frame, w, h, 3);
    ...
    addText(frame, "headline", "5 Signs You Need\nDigital Security", resolveFont(BEBAS), Math.round(w * 0.065), C.white, Math.round(w * 0.08), Math.round(h * 0.28), {
        maxWidth: Math.round(w * 0.84),
        lineHeight: 115,
        align: "CENTER",
        effect: "gradientFill",
    });
    addText(frame, "description", "Swipe to learn more", INTER_REG, Math.round(w * 0.025), C.gray, Math.round(w * 0.08), Math.round(h * 0.6), {
        maxWidth: Math.round(w * 0.84),
        align: "CENTER",
    });
    // Swipe indicator
    addText(frame, "swipe", "SWIPE \u2192", INTER_SEMI, Math.round(w * 0.018), C.dim, 0, Math.round(h * 0.82), {
        letterSpacing: 20,
    }).x = Math.round(cx - 40);
    // Slide dots
    for (let i = 0; i < 5; i++) {
        addEllipse(frame, `dot-${i}`, 8, 8, Math.round(cx - 32 + i * 16), Math.round(h * 0.92), solid(i === 0 ? C.red : C.dim));
    }
}
function buildCarouselSlide(frame, w, h) {
    const pad = Math.round(w * 0.08);
    addPremiumBg(frame, w, h, 4);
    // Slide number
    addText(frame, "slide_number", "02", INTER_BOLD, Math.round(w * 0.1), C.red, pad, Math.round(h * 0.06), {
        opacity: 0.3,
    });
    addText(frame, "headline", "Paper Logbooks", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.055), C.white, pad, Math.round(h * 0.2), {
        maxWidth: Math.round(w * 0.84),
        lineHeight: 115,
        effect: "lifted",
    });
    addText(frame, "description", "Still tracking incidents on paper? You're losing 4 hours per site per week.", INTER_REG, Math.round(w * 0.025), C.gray, pad, Math.round(h * 0.38), { maxWidth: Math.round(w * 0.84), lineHeight: 155 });
    // Image area (bottom half)
    addRect(frame, "image-area", Math.round(w * 0.84), Math.round(h * 0.35), pad, Math.round(h * 0.56), solid(C.card), 12);
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.016), C.dim, 0, Math.round(h * 0.72), {
        align: "CENTER",
    }).x = Math.round(w / 2 - 55);
    // Progress dots
    const cx = Math.round(w / 2);
    for (let i = 0; i < 5; i++) {
        addEllipse(frame, `dot-${i}`, 8, 8, Math.round(cx - 32 + i * 16), Math.round(h * 0.95), solid(i === 1 ? C.red : C.dim));
    }
}
function buildCarouselCTA(frame, w, h) {
    const cx = Math.round(w / 2);
    addPremiumBg(frame, w, h, 4);
    addRect(frame, "accent", w, 6, 0, 0, solid(C.red));
    const brand = addBrand(frame, 0, Math.round(h * 0.08), Math.round(w * 0.022));
    brand.x = Math.round(cx - brand.width / 2);
    addText(frame, "headline", "Ready to\nUpgrade?", resolveFont(BEBAS), Math.round(w * 0.07), C.white, Math.round(w * 0.08), Math.round(h * 0.28), {
        maxWidth: Math.round(w * 0.84),
        lineHeight: 110,
        align: "CENTER",
        effect: "glow",
    });
    addText(frame, "description", "Join 500+ security firms on Commando360", INTER_REG, Math.round(w * 0.025), C.gray, Math.round(w * 0.08), Math.round(h * 0.55), {
        maxWidth: Math.round(w * 0.84),
        align: "CENTER",
    });
    const btn = addCtaButton(frame, Math.round(w * 0.03), 0, Math.round(h * 0.7), Math.round(w * 0.05), Math.round(w * 0.022), undefined, true);
    btn.x = Math.round(cx - btn.width / 2);
    // Slide dots
    for (let i = 0; i < 5; i++) {
        addEllipse(frame, `dot-${i}`, 8, 8, Math.round(cx - 32 + i * 16), Math.round(h * 0.92), solid(i === 4 ? C.red : C.dim));
    }
}
// ══════════════════════════════════════════════════════════════
// IMAGE POST LAYOUTS (3 styles)
// ══════════════════════════════════════════════════════════════
function buildImageBottom(frame, w, h) {
    const splitY = Math.round(h * 0.6);
    const pad = Math.round(w * 0.05);
    addPremiumBg(frame, w, h, 0);
    // Image area (top 60%)
    addRect(frame, "image-area", w, splitY, 0, 0, solid(C.card));
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.016), C.dim, 0, Math.round(splitY / 2 - 8), {
        align: "CENTER",
        maxWidth: w,
    });
    // Bottom text bar
    ...
    addText(frame, "headline", "Your Headline Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.032), C.white, pad, splitY + Math.round((h - splitY) * 0.15), {
        maxWidth: Math.round(w * 0.7),
        lineHeight: 120,
        effect: "lifted",
    });
    addText(frame, "description", "Benefit-focused description.", INTER_REG, Math.round(w * 0.018), C.gray, pad, splitY + Math.round((h - splitY) * 0.55), {
        maxWidth: Math.round(w * 0.65),
    });
    addBrand(frame, w - pad - 90, h - pad - 12, Math.round(w * 0.011), true);
}
function buildImageTop(frame, w, h) {
    ...
    addText(frame, "headline", "Your Headline Here", resolveFont(BEBAS), Math.round(w * 0.035), C.white, pad, Math.round(barH * 0.4), {
        maxWidth: Math.round(w * 0.85),
        lineHeight: 120,
    });
    // Image area (bottom 72%)
    addRect(frame, "image-area", w, h - barH, 0, barH, solid(C.card));
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.016), C.dim, 0, Math.round(barH + (h - barH) / 2 - 8), {
        align: "CENTER",
        maxWidth: w,
    });
    // CTA overlay bottom-right
    addCtaButton(frame, Math.round(w * 0.018), w - pad - 140, h - pad - 36, Math.round(w * 0.02), Math.round(w * 0.01), undefined, true);
}
function buildImageFull(frame, w, h) {
    const pad = Math.round(w * 0.05);
    addPremiumBg(frame, w, h, 2);
    // Full image area
    addRect(frame, "image-area", w, h, 0, 0, solid(C.card));
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.02), C.dim, 0, Math.round(h * 0.45), {
        align: "CENTER",
        maxWidth: w,
    });
    // Dark scrim overlay at bottom
    addRect(frame, "scrim", w, Math.round(h * 0.35), 0, Math.round(h * 0.65), solid(C.bg, 0.8));
    addText(frame, "headline", "Your Headline Here", resolveFont(BEBAS), Math.round(w * 0.035), C.white, pad, Math.round(h * 0.72), {
        maxWidth: Math.round(w * 0.75),
        lineHeight: 120,
        effect: "lifted",
    });
    const btn = addCtaButton(frame, Math.round(w * 0.018), w - pad - 130, Math.round(h * 0.88), Math.round(w * 0.02), Math.round(w * 0.01), undefined, true);
    // adjust if off screen
    if (btn.x + btn.width > w)
        btn.x = w - pad - btn.width;
    addBrand(frame, pad, Math.round(h * 0.04), Math.round(w * 0.012));
}
// ══════════════════════════════════════════════════════════════
// STATS LAYOUTS (3 styles)
// ══════════════════════════════════════════════════════════════
function buildBigStat(frame, w, h) {
    const cx = Math.round(w / 2);
    addPremiumBg(frame, w, h, 0);
    addBrand(frame, Math.round(w * 0.05), Math.round(h * 0.06), Math.round(w * 0.013));
    // Context headline
    addText(frame, "headline", "Reduction in incidents", INTER_MED, Math.round(w * 0.02), C.gray, 0, Math.round(h * 0.18), {
        maxWidth: Math.round(w * 0.8),
        align: "CENTER",
    }).x = Math.round(cx - w * 0.4);
    // Massive stat
    const stat = addText(frame, "stat", "73%", resolveFont(BEBAS), Math.round(w * 0.16), C.white, 0, Math.round(h * 0.28), { effect: "glow" });
    stat.x = Math.round(cx - stat.width / 2);
    // Stat label
    const label = addText(frame, "stat_label", "fewer incidents", INTER_SEMI, Math.round(w * 0.025), C.red, 0, Math.round(h * 0.62));
    label.x = Math.round(cx - label.width / 2);
    addText(frame, "description", "AI-powered command centers catch threats before they escalate.", INTER_REG, Math.round(w * 0.018), C.gray, 0, Math.round(h * 0.74), {
        maxWidth: Math.round(w * 0.65),
        align: "CENTER",
    }).x = Math.round(cx - w * 0.325);
    addCtaButton(frame, Math.round(w * 0.018), 0, Math.round(h * 0.86), Math.round(w * 0.025), Math.round(w * 0.012), undefined, true).x = Math.round(cx - 70);
    // Red accent lines
    const lineW = Math.round(w * 0.08);
    addRect(frame, "line-l", lineW, 2, Math.round(cx - w * 0.25), Math.round(h * 0.68), solid(C.red, 0.3));
    addRect(frame, "line-r", lineW, 2, Math.round(cx + w * 0.17), Math.round(h * 0.68), solid(C.red, 0.3));
}
function buildBeforeAfter(frame, w, h) {
    const cx = Math.round(w / 2);
    const pad = Math.round(w * 0.05);
    ...
    // Main headline
    addText(frame, "headline", "Before vs After Commando360.ai", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.03), C.white, 0, Math.round(h * 0.15), {
        maxWidth: Math.round(w * 0.9),
        align: "CENTER",
    }).x = Math.round(cx - w * 0.45);
    // Red center divider
    addRect(frame, "divider", 2, Math.round(h * 0.55), cx - 1, Math.round(h * 0.28), solid(C.red, 0.5));
    // Left column — Before
    addText(frame, "left_headline", "BEFORE", INTER_BOLD, Math.round(w * 0.016), C.dim, pad, Math.round(h * 0.28), {
        letterSpacing: 15,
    });
    addText(frame, "left_description", "Paper logbooks, missed patrols, buddy punching, no real-time visibility", INTER_REG, Math.round(w * 0.018), C.gray, pad, Math.round(h * 0.36), {
        maxWidth: colW,
        lineHeight: 160,
    });
    // Right column — After
    const rightX = cx + pad;
    addText(frame, "right_headline", "WITH Commando360.ai", INTER_BOLD, Math.round(w * 0.016), C.red, rightX, Math.round(h * 0.28), {
        letterSpacing: 15,
    });
    addText(frame, "right_description", "GPS tracking, face ID attendance, real-time command center, auto compliance", INTER_REG, Math.round(w * 0.018), C.white, rightX, Math.round(h * 0.36), {
        maxWidth: colW,
        lineHeight: 160,
    });
    addCtaButton(frame, Math.round(w * 0.018), 0, Math.round(h * 0.84), Math.round(w * 0.025), Math.round(w * 0.012), undefined, true).x = Math.round(cx - 70);
}
function buildStatRow(frame, w, h) {
    const cx = Math.round(w / 2);
    const pad = Math.round(w * 0.06);
    ...
    addText(frame, "headline", "Commando360 by the Numbers", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.03), C.white, 0, Math.round(h * 0.18), {
        maxWidth: Math.round(w * 0.9),
        align: "CENTER",
        effect: "gradientFill",
    }).x = Math.round(cx - w * 0.45);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const sv = addText(frame, stats[i].name, stats[i].value, INTER_BOLD, Math.round(w * 0.055), C.white, 0, startY, {
            align: "CENTER",
        });
        sv.x = Math.round(colCx - sv.width / 2);
        const sl = addText(frame, stats[i].label, stats[i].labelText, INTER_REG, Math.round(w * 0.015), C.gray, 0, startY + Math.round(h * 0.15), {
            align: "CENTER",
        });
        sl.x = Math.round(colCx - sl.width / 2);
        // Accent dot above
        addEllipse(frame, `stat-dot-${i}`, 6, 6, colCx - 3, startY - 16, solid(C.red));
    }
    addText(frame, "description", "The platform trusted by India's leading security companies", INTER_REG, Math.round(w * 0.017), C.gray, 0, Math.round(h * 0.72), {
        maxWidth: Math.round(w * 0.7),
        align: "CENTER",
...
// VIDEO THUMBNAIL
// ══════════════════════════════════════════════════════════════
function buildVideoThumb(frame, w, h) {
    const cx = Math.round(w / 2);
    ...
    addEllipse(frame, "play-circle", playSize, playSize, Math.round(cx - playSize / 2), Math.round(cy - playSize / 2), solid(C.white, 0.9));
    // Play triangle (using text character)
    const play = addText(frame, "play-icon", "\u25B6", INTER_REG, Math.round(playSize * 0.4), C.bg, 0, 0);
    play.x = Math.round(cx - play.width / 2 + 2);
    play.y = Math.round(cy - play.height / 2);
    // Duration badge (top right)
    const dur = addText(frame, "duration", "2:30", INTER_SEMI, Math.round(w * 0.016), C.white, 0, pad);
    const durBg = addRect(frame, "dur-bg", dur.width + 16, dur.height + 8, w - pad - dur.width - 16, pad - 4, solid(C.bg, 0.7), 4);
    // bring text above bg — move dur after bg in the tree
    dur.x = durBg.x + 8;
    dur.y = durBg.y + 4;
    // Bottom text
    addRect(frame, "bottom-scrim", w, Math.round(h * 0.25), 0, Math.round(h * 0.75), solid(C.bg, 0.75));
    addText(frame, "headline", "Your Video Title Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.03), C.white, pad, Math.round(h * 0.8), {
        maxWidth: Math.round(w * 0.8),
        effect: "lifted",
    });
...
// QUOTE CARD
// ══════════════════════════════════════════════════════════════
function buildQuoteCard(frame, w, h) {
    const cx = Math.round(w / 2);
    const pad = Math.round(w * 0.08);
    addPremiumBg(frame, w, h, 3);
    addRect(frame, "accent", w, 4, 0, 0, solid(C.red));
    // Big decorative quote mark
    addText(frame, "quote-mark", "\u201C", INTER_BOLD, Math.round(w * 0.18), C.red, pad, Math.round(h * 0.04), {
        opacity: 0.2,
    });
    addBrand(frame, w - pad - 100, Math.round(h * 0.06), Math.round(w * 0.012));
    // Quote text
    addText(frame, "quote", "Commando360 transformed how we manage our 50-site security operation. What took hours now takes minutes.", resolveFont(BEBAS), Math.round(w * 0.025), C.white, pad, Math.round(h * 0.28), { maxWidth: Math.round(w * 0.84), lineHeight: 170, align: "CENTER" });
    // Red divider
    const divW = Math.round(w * 0.08);
    addRect(frame, "divider", divW, 2, Math.round(cx - divW / 2), Math.round(h * 0.62), solid(C.red));
    // Author
    addText(frame, "author", "Rajesh Kumar, CEO\nSecureForce India", INTER_SEMI, Math.round(w * 0.018), C.gray, 0, Math.round(h * 0.68), {
        maxWidth: Math.round(w * 0.7),
        align: "CENTER",
        lineHeight: 160,
    }).x = Math.round(cx - w * 0.35);
    // Optional headline/context
    addText(frame, "headline", "Client Testimonial", INTER_REG, Math.round(w * 0.014), C.dim, 0, Math.round(h * 0.87), {
        align: "CENTER",
        letterSpacing: 15,
    }).x = Math.round(cx - 55);
}
...
// ══════════════════════════════════════════════════════════════
function buildStory(frame, w, h) {
    const pad = Math.round(w * 0.08);
    addPremiumBg(frame, w, h, 5);
    addRect(frame, "accent", w, 4, 0, 0, solid(C.red));
    const brand = addBrand(frame, 0, Math.round(h * 0.06), Math.round(w * 0.018));
    brand.x = Math.round((w - brand.width) / 2);
    addText(frame, "headline", "Your Headline\nGoes Here", resolveFont(BEBAS), Math.round(w * 0.065), C.white, pad, Math.round(h * 0.3), {
        maxWidth: w - pad * 2,
        lineHeight: 115,
        align: "CENTER",
        effect: "gradientFill",
    });
    addText(frame, "description", "Your description text goes here.\nKeep it benefit-focused.", INTER_REG, Math.round(w * 0.028), C.gray, pad, Math.round(h * 0.48), {
        maxWidth: w - pad * 2,
        lineHeight: 155,
        align: "CENTER",
    });
    const btn = addCtaButton(frame, Math.round(w * 0.028), 0, Math.round(h * 0.65), Math.round(w * 0.05), Math.round(w * 0.02), undefined, true);
    btn.x = Math.round((w - btn.width) / 2);
    for (let i = 0; i < 3; i++) {
        addEllipse(frame, `dot-${i}`, 6, 6, Math.round(w / 2 - 20 + i * 14), Math.round(h * 0.88), solid(i === 0 ? C.red : C.dim));
    }
}
function buildCompact(frame, w, h) {
    ...
    addText(frame, "headline", "Your Headline", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.065), C.white, left, Math.round(h * 0.25), {
        maxWidth: w - left - 16,
        lineHeight: 120,
        effect: "lifted",
    });
    addText(frame, "description", "Benefit-focused copy.", INTER_REG, Math.round(w * 0.04), C.gray, left, Math.round(h * 0.55), {
        maxWidth: w - left - 16,
        lineHeight: 145,
    });
    addCtaButton(frame, Math.round(w * 0.038), left, Math.round(h * 0.78), Math.round(w * 0.04), Math.round(w * 0.025), undefined, true);
}
function buildLeaderboard(frame, w, h) {
    addPremiumBg(frame, w, h, 5);
    addRect(frame, "accent", 4, h, 0, 0, solid(C.red));
    addBrand(frame, 24, Math.round((h - 14) / 2), 14, true);
    addText(frame, "headline", "Your Headline Here", INTER_BOLD, 18, C.white, 100, Math.round((h - 22) / 2));
...
function buildEditorialOverlay(frame, w, h) {
    var pad = Math.round(w * 0.06);
    // Full-bleed image area
    addRect(frame, "image-area", w, h, 0, 0, solid(C.card));
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.018), C.dim, 0, Math.round(h * 0.45), { align: "CENTER", maxWidth: w });
    ...
    addRect(frame, "accent-line", Math.round(w * 0.08), 3, pad, Math.round(h * 0.76), solid(C.red));
    addText(frame, "headline", "Your Headline Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.04), FIXED_WHITE, pad, Math.round(h * 0.79), { maxWidth: Math.round(w * 0.7), lineHeight: 115, effect: "lifted" });
    addText(frame, "description", "Supporting caption or tagline goes here", INTER_REG, Math.round(w * 0.016), { r: 0.85, g: 0.85, b: 0.85 }, pad, Math.round(h * 0.91), { maxWidth: Math.round(w * 0.65), lineHeight: 150 });
    addBrand(frame, w - pad - Math.round(w * 0.04), Math.round(h * 0.04), Math.round(w * 0.012));
}
// Magazine cover: full image with top headline + subtitle
function buildMagazineCover(frame, w, h) {
    var pad = Math.round(w * 0.06);
    // Full-bleed image
    addRect(frame, "image-area", w, h, 0, 0, solid(C.card));
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.018), C.dim, 0, Math.round(h * 0.45), { align: "CENTER", maxWidth: w });
    ...
    frame.appendChild(scrim);
    addBrand(frame, pad, Math.round(h * 0.04), Math.round(w * 0.012));
    addText(frame, "headline", "Magazine\nStyle Hero", resolveFont(BEBAS), Math.round(w * 0.055), FIXED_WHITE, pad, Math.round(h * 0.1), { maxWidth: Math.round(w * 0.8), lineHeight: 105, effect: "lifted" });
    addText(frame, "description", "Subheadline or date", INTER_MED, Math.round(w * 0.016), { r: 0.9, g: 0.9, b: 0.9 }, pad, Math.round(h * 0.28), { maxWidth: Math.round(w * 0.6) });
    // Bottom CTA
    addCtaButton(frame, Math.round(w * 0.018), pad, Math.round(h * 0.88), Math.round(w * 0.022), Math.round(w * 0.012), undefined, true);
}
// Center-cropped image with generous white/dark border and centered text below
function buildFramedPhoto(frame, w, h) {
    ...
    var imgRect = addRect(frame, "image-area", imgW, imgH, pad, Math.round(h * 0.1), solid(C.card), 12);
    addText(frame, "image-label", "DROP IMAGE HERE", INTER_MED, Math.round(w * 0.018), C.dim, 0, Math.round(h * 0.1 + imgH * 0.45), { align: "CENTER", maxWidth: w });
    // Text area below image
    var textY = Math.round(h * 0.1) + imgH + Math.round(h * 0.04);
    addText(frame, "headline", "Your Headline Here", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.032), C.white, 0, textY, { maxWidth: w, align: "CENTER", lineHeight: 120 });
    addText(frame, "description", "A brief description or caption for this image", INTER_REG, Math.round(w * 0.016), C.gray, 0, textY + Math.round(h * 0.07), { maxWidth: w, align: "CENTER", lineHeight: 150 });
}
// ══════════════════════════════════════════════════════════════
...
function buildBoldStatement(frame, w, h) {
    var pad = Math.round(w * 0.08);
    ...
    addBrand(frame, pad, Math.round(h * 0.06), Math.round(w * 0.012));
    addText(frame, "headline", "Bold Statement\nGoes Here", resolveFont(BEBAS), Math.round(w * 0.06), FIXED_WHITE, 0, Math.round(h * 0.3), { maxWidth: w, align: "CENTER", lineHeight: 105, effect: "lifted" });
    addText(frame, "description", "Supporting line that adds context", INTER_MED, Math.round(w * 0.018), { r: 1, g: 0.85, b: 0.85 }, 0, Math.round(h * 0.65), { maxWidth: w, align: "CENTER", lineHeight: 150 });
    addCtaButton(frame, Math.round(w * 0.018), 0, Math.round(h * 0.8), Math.round(w * 0.022), Math.round(w * 0.012), "Learn More", true);
...
function buildColorAnnouncement(frame, w, h) {
    var pad = Math.round(w * 0.08);
    ...
    addEllipse(frame, "icon-area", circleSize, circleSize, cx, cy, solid({ r: 1, g: 1, b: 1 }, 0.12));
    addText(frame, "icon-label", "ICON", INTER_BOLD, Math.round(circleSize * 0.2), { r: 1, g: 1, b: 1 }, 0, cy + Math.round(circleSize * 0.35), { maxWidth: w, align: "CENTER", opacity: 0.5 });
    addText(frame, "headline", "Announcing Something", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.038), FIXED_WHITE, 0, Math.round(h * 0.45), { maxWidth: w, align: "CENTER", lineHeight: 120, effect: "lifted" });
    addText(frame, "description", "Details about this announcement or feature update", INTER_REG, Math.round(w * 0.016), { r: 1, g: 0.8, b: 0.8 }, 0, Math.round(h * 0.6), { maxWidth: Math.round(w * 0.75), align: "CENTER", lineHeight: 155 });
    // Center the description
    addCtaButton(frame, Math.round(w * 0.018), 0, Math.round(h * 0.78), Math.round(w * 0.022), Math.round(w * 0.012), undefined, true);
}
// Solid color with large number/stat — bold data point post
function buildColorStat(frame, w, h) {
    var pad = Math.round(w * 0.08);
    ...
    addText(frame, "stat", "97%", resolveFont(BEBAS), Math.round(w * 0.14), FIXED_WHITE, 0, Math.round(h * 0.2), { maxWidth: w, align: "CENTER", lineHeight: 100, effect: "lifted" });
    addText(frame, "headline", "of teams report better outcomes", resolveFont(JAKARTA_XBOLD), Math.round(w * 0.028), FIXED_WHITE, 0, Math.round(h * 0.55), { maxWidth: Math.round(w * 0.8), align: "CENTER", lineHeight: 130 });
    addText(frame, "description", "Source: Industry Report 2024", INTER_REG, Math.round(w * 0.014), { r: 1, g: 0.8, b: 0.8 }, 0, Math.round(h * 0.72), { maxWidth: w, align: "CENTER", opacity: 0.7 });
    // Decorative dots at bottom
    var dotSize = Math.round(w * 0.008);
    var dotsX = Math.round(w / 2) - Math.round(dotSize * 4);
    addAccentDots(frame, 4, dotSize, dotsX, Math.round(h * 0.88), Math.round(dotSize * 2));
}
function getDeviceColors(colorScheme) {
...
function generateiPhoneSVG(w, h, dc) {
    ...
    var s = '<svg xmlns="http://www.w3.org/2000/svg" width="' +
        w +
        '" height="' +
        h +
        '">';
...
function getPhoneScreenRect(w, h) {
    var bx = Math.max(4, Math.round(w * 0.021));
    var by = Math.max(5, Math.round(h * 0.013));
...
function generateMacBookSVG(w, h, dc) {
...
function getMacBookScreenRect(w, h) {
    var housingW = Math.round(w * 0.87);
    var housingH = Math.round(h * 0.935);
    var housingX = Math.round((w - housingW) / 2);
    var bezT = Math.round(housingH * 0.042);
...
function generateBrowserSVG(w, h, dark) {
...
function getBrowserScreenRect(w, h) {
    var titleH = Math.max(32, Math.round(h * 0.058));
...
function generateWindowsBrowserSVG(w, h, dark) {
    var titleH = Math.max(32, Math.round(h * 0.058));
    var barBg = dark ? "#2b2b2b" : "#dee1e6";
    var contentBg = dark ? "#1a1a1a" : "#ffffff";
    var urlBg = dark ? "#1a1a1a" : "#ffffff";
...
function getWindowsBrowserScreenRect(w, h) {
    var titleH = Math.max(32, Math.round(h * 0.058));
    return { x: 0, y: titleH + 1, w: w, h: h - titleH - 1 };
}
// ── Device Placement Helper ─────────────────────────────────
// Creates a device frame with an image placeholder at screen position
function addDeviceMockup(parent, device, x, y, deviceH, colorScheme, rot) {
    ...
    screen.y = screenR.y;
    screen.fills = [
        solid(isDark ? { r: 0.05, g: 0.06, b: 0.07 } : { r: 0.1, g: 0.11, b: 0.13 }),
    ];
    if (device === "iphone") {
...
function buildMockupPhoneHero(parent, w, h, color) {
    addPremiumBg(parent, w, h, 0);
    var dH = Math.round(h * 0.78);
    var dW = Math.round(dH * 0.483);
    addDeviceMockup(parent, "iphone", Math.round((w - dW) / 2), Math.round((h - dH) / 2), dH, color);
}
function buildMockupLaptopHero(parent, w, h, color) {
    addPremiumBg(parent, w, h, 1);
    var dH = Math.round(h * 0.72);
    var dW = Math.round(dH * 1.55);
    addDeviceMockup(parent, "macbook", Math.round((w - dW) / 2), Math.round((h - dH) / 2), dH, color);
}
function buildMockupBrowserHero(parent, w, h, color) {
    addPremiumBg(parent, w, h, 2);
    var dH = Math.round(h * 0.82);
    var dW = Math.round(dH * 1.5);
    addDeviceMockup(parent, "browser", Math.round((w - dW) / 2), Math.round((h - dH) / 2), dH, color);
}
function buildMockupPhoneTrio(parent, w, h, color) {
    ...
    addDeviceMockup(parent, "iphone", cx - spread - Math.round(sideW / 2), cy - Math.round(sideH / 2) + Math.round(h * 0.02), sideH, color, 12);
    // Right phone (behind, rotated)
    addDeviceMockup(parent, "iphone", cx + spread - Math.round(sideW / 2), cy - Math.round(sideH / 2) + Math.round(h * 0.02), sideH, color, -12);
    // Center phone (front, on top — added last for z-order)
    addDeviceMockup(parent, "iphone", cx - Math.round(centerW / 2), cy - Math.round(centerH / 2), centerH, color);
}
function buildMockupPhoneLaptop(parent, w, h, color) {
...
function buildMockupCascade(parent, w, h, color) {
    ...
    addDeviceMockup(parent, "iphone", cx - offsetX - Math.round(phW / 2), cy - offsetY - Math.round(phH / 2), phH, color);
    // Front phone (offset down-right)
    addDeviceMockup(parent, "iphone", cx + offsetX - Math.round(phW / 2), cy + offsetY - Math.round(phH / 2), phH, color);
}
function buildMockupResponsive(parent, w, h, color) {
    addPremiumBg(parent, w, h, 0);
...
function buildMockupWindowsBrowserHero(parent, w, h, color) {
    addPremiumBg(parent, w, h, 2);
    var dH = Math.round(h * 0.82);
    var dW = Math.round(dH * 1.5);
    addDeviceMockup(parent, "windows-browser", Math.round((w - dW) / 2), Math.round((h - dH) / 2), dH, color);
}
...
function buildMockupPostSplit(parent, w, h, color, postDevice) {
    ...
    addText(parent, "headline", "YOUR HEADLINE HERE", INTER_BOLD, headSz, C.white, padX, padY, {
        maxWidth: midX - padX * 2,
        lineHeight: 120,
    });
    // Subtext
    var subSz = Math.max(10, Math.round(h * 0.022));
    addText(parent, "subtext", "Supporting copy goes here. Describe the feature shown in the mockup.", INTER_REG, subSz, C.gray, padX, padY + Math.round(headSz * 3), {
        maxWidth: midX - padX * 2,
        lineHeight: 150,
    });
    // Red accent line
    addRect(parent, "accent", Math.round(w * 0.06), 3, padX, padY + Math.round(headSz * 2.2), solid(C.red));
    // Device on right side
    var devH = Math.round(safeH * 0.72);
    var isWide = postDevice === "macbook" ||
        postDevice === "browser" ||
...
function buildMockupPostCentered(parent, w, h, color, postDevice) {
    ...
    addText(parent, "headline", "YOUR HEADLINE HERE", INTER_BOLD, headSz, C.white, padX, topPad, {
        maxWidth: w - padX * 2,
        align: "CENTER",
    });
    // Subtext
    var subSz = Math.max(10, Math.round(h * 0.02));
    addText(parent, "subtext", "Brief description of the product feature being showcased.", INTER_REG, subSz, C.gray, padX, topPad + Math.round(headSz * 2), {
        maxWidth: w - padX * 2,
        align: "CENTER",
        lineHeight: 150,
    });
...
function buildMockupPostAngled(parent, w, h, color, postDevice) {
    ...
    // Subtext
    var subSz = Math.max(10, Math.round(h * 0.02));
    addText(parent, "subtext", "Feature description that sells the product.", INTER_REG, subSz, C.gray, padX, padY + Math.round(headSz * 2.5), {
        maxWidth: Math.round(w * 0.45),
        lineHeight: 150,
    });
    // Red accent
    addRect(parent, "accent", Math.round(w * 0.06), 3, padX, padY + Math.round(headSz * 2), solid(C.red));
    ...
    addDeviceMockup(parent, postDevice, devX, devY, devH, color, isWide ? -3 : -8);
    addBrandFooter(parent, w, h, footerH, brandLogo || undefined);
}
function buildMockupPostStory(parent, w, h, color, postDevice) {
    ...
    addText(parent, "headline", "YOUR HEADLINE HERE", INTER_BOLD, headSz, C.white, padX, textY, {
        maxWidth: w - padX * 2,
        align: "CENTER",
        lineHeight: 120,
    });
    var subSz = Math.max(11, Math.round(h * 0.018));
    addText(parent, "subtext", "Describe the feature shown above. Perfect for stories and vertical content.", INTER_REG, subSz, C.gray, padX, textY + Math.round(headSz * 2.5), {
        maxWidth: w - padX * 2,
        align: "CENTER",
        lineHeight: 150,
    });
    ...
    addRect(parent, "accent", accentW, 3, Math.round((w - accentW) / 2), textY + Math.round(headSz * 2), solid(C.red));
    addBrandFooter(parent, w, h, footerH, brandLogo || undefined);
}
// ── Phone + Desktop combo post ──────────────────────────────
function buildMockupPostDual(parent, w, h, color, desktopDevice) {
    addPremiumBg(parent, w, h, 5);
    var footerH = Math.round(h * 0.09);
    var safeH = h - footerH;
    ...
    addText(parent, "headline", "YOUR HEADLINE HERE", INTER_BOLD, headSz, C.white, textX, textY, {
        maxWidth: w - textX - padX,
        lineHeight: 120,
    });
    var subSz = Math.max(9, Math.round(h * 0.018));
    addText(parent, "subtext", "Available on all devices.", INTER_REG, subSz, C.gray, textX, textY + Math.round(headSz * 2), {
        maxWidth: w - textX - padX,
        lineHeight: 150,
    });
    addBrandFooter(parent, w, h, footerH, brandLogo || undefined);
...
function buildMockupPostDualStory(parent, w, h, color, desktopDevice) {
    addPremiumBg(parent, w, h, 2);
    var footerH = Math.round(h * 0.05);
    var safeH = h - footerH;
    var padX = Math.round(w * 0.06);
    ...
    addText(parent, "subtext", "Works everywhere you do.", INTER_REG, subSz, C.gray, padX, textY + Math.round(headSz * 2.5), {
        maxWidth: w - padX * 2,
        align: "CENTER",
        lineHeight: 150,
    });
...
async function createMockups(layouts, deviceColor, themes, platforms, postDeviceFocus, desktopStyle, logoImage, fullLogoImage) {
    await Promise.all([
    ...
    var totalCount = layouts.length * themes.length +
    ...
    if (platforms.length > 0 && applicableStyles.length > 0) {
        ...
        for (var pi = 0; pi < platforms.length; pi++) {
            ...
            for (var si = 0; si < applicableStyles.length; si++) {
                ...
                if (style.forStory && !isStory)
                    continue;
                if (!style.forStory && isStory)
                    continue;
                // Resolve the device string to pass to the builder
                var devForBuild = style.device === "phone"
                    ? "iphone"
                ...
                for (var tti = 0; tti < themes.length; tti++) {
                    ...
                    applyTheme(isDark2);
                    var tSuffix2 = themes.length > 1 ? (isDark2 ? " (Dark)" : " (Light)") : "";
...
async function createTemplates(platforms, contentTypes, brandFrame, themes, bgPattern, bgDensity, qrImage, logoImageBytes, fullLogoImageBytes, patternImageBytes, patternSvgStr) {
    ...
    for (const key of platforms) {
        ...
        if (isStandard(p)) {
            ...
            if (contentTypes.indexOf("image") >= 0)
                totalCount += IMAGE_STYLES.length;
            if (contentTypes.indexOf("stats") >= 0)
                totalCount += STATS_STYLES.length;
            if (contentTypes.indexOf("video") >= 0)
                totalCount += VIDEO_STYLES.length;
            if (contentTypes.indexOf("quote") >= 0)
                totalCount += QUOTE_STYLES.length;
    ...
    for (const theme of themes) {
        ...
        for (const key of platforms) {
            ...
            if (isStandard(p)) {
                ...
                if (contentTypes.indexOf("image") >= 0)
                    typesToBuild.push(IMAGE_STYLES);
                if (contentTypes.indexOf("stats") >= 0)
                    typesToBuild.push(STATS_STYLES);
                if (contentTypes.indexOf("video") >= 0)
                    typesToBuild.push(VIDEO_STYLES);
                if (contentTypes.indexOf("quote") >= 0)
                    typesToBuild.push(QUOTE_STYLES);
                if (contentTypes.indexOf("fullpage") >= 0)
                ...
                for (const variants of typesToBuild) {
                    for (const v of variants) {
                        ...
                        const f = createFrame(p.w, p.h, `Commando360.ai \u2014 ${p.label} \u2014 ${v.suffix}${tSuffix}`, v.build, useBrand, logoImg);
                        f.x = xPos;
                        f.y = yPos;
                ...
                const f = createFrame(p.w, p.h, `Commando360.ai \u2014 ${p.label}${tSuffix}`, buildFn, brandFrame, logoImg);
                f.x = xPos;
                f.y = yPos;
                figma.currentPage.appendChild(f);
                frames.push(f);
        ...
        if (contentTypes.indexOf("carousel") >= 0) {
            ...
            for (const v of CAROUSEL_SLIDES) {
                const useBrand = brandFrame && v.brandFrame !== false;
                const f = createFrame(1080, 1080, `Commando360.ai \u2014 ${v.suffix}${tSuffix}`, v.build, useBrand, logoImg);
                f.x = xPos;
                f.y = yPos;
                figma.currentPage.appendChild(f);
                frames.push(f);
    ...
    if (qrImage && qrImage.length > 0) {
        ...
        for (const f of frames) {
            const qrRect = f.findOne((n) => n.name === "qr-code" && n.type === "RECTANGLE");
            if (qrRect) {
                qrRect.fills = [
...
function createFrame(w, h, name, buildFn, brandFrame, logoImage) {
...
function getBrochureColors(dark) {
    if (dark) {
        return {
            bg: { r: 0.031, g: 0.035, b: 0.039 },
            heading: { r: 1, g: 1, b: 1 },
...
var B = {
    ...
    DARK_BG: { r: 0.102, g: 0.106, b: 0.118 },
    DARK_CARD: { r: 0.14, g: 0.145, b: 0.16 },
    LIGHT_GRAY: { r: 0.65, g: 0.65, b: 0.67 },
};
function addBrochureBlock(parent, name, w, h, x, y, color, radius) {
...
function addBrochurePlaceholder(parent, name, w, h, x, y, bc, radius) {
...
function addLaptopFrame(parent, name, laptopW, x, y, bc, dark) {
...
function addBrochureIconCircle(parent, size, x, y, bc, filled) {
...
function addBrochureDarkCard(parent, name, w, h, x, y, radius) {
...
function addBrochureFeatureItem(parent, index, x, y, maxW, titleText, descText, bc, w, // page width for proportional sizing
opts) {
    ...
    addText(parent, "icon-" + index + "-title", titleText, INTER_SEMI, Math.round(w * B.FEAT_TITLE), titleColor, textX, y + Math.round(iconSize * 0.05), { maxWidth: textW });
    var descNode = addText(parent, "icon-" + index + "-desc", descText, INTER_REG, Math.round(w * B.BODY_SIZE), descColor, textX, y + Math.round(w * B.FEAT_TITLE) + Math.round(iconSize * 0.15), { maxWidth: textW, lineHeight: 150 });
    return (iconSize +
...
// ══════════════════════════════════════════════════════════════
function buildBrochureHeroFeature(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    // Subtitle
    addText(frame, "subtitle", "Manage your operations with clarity, control, and confidence.", INTER_REG, Math.round(w * B.SUBTITLE_SIZE), bc.subtext, margin, Math.round(h * 0.08), { maxWidth: contentW, align: "CENTER" });
    // 2x2 icon grid
    ...
    for (var fi = 0; fi < 4; fi++) {
        ...
        addBrochureFeatureItem(frame, fi + 1, fCol, gridY + fRow * rowGap, colW, featureTitles[fi], featureDescs[fi], bc, w);
    }
    ...
    var calloutX = margin;
    addBrochureFeatureItem(frame, 5, calloutX, calloutY, Math.round(contentW * 0.45), "Additional Feature", "Capture performance data through structured field surveys.", bc, w, { darkBg: true });
    addBrochureFeatureItem(frame, 6, margin + Math.round(contentW * 0.54), calloutY, Math.round(contentW * 0.45), "Another Feature", "Digital verification with QR codes and controlled access.", bc, w, { darkBg: true });
}
function buildBrochureSplitLeft(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    ...
    // Title
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, textX, textY, { maxWidth: textW, letterSpacing: 3 });
    // Red accent line
    addRect(frame, "accent-line", Math.round(w * 0.08), 3, textX, textY + Math.round(w * B.TITLE_SIZE) + Math.round(h * 0.015), solid(bc.accent));
    // Subtitle
    addText(frame, "subtitle", "Brief description of what this feature does.", INTER_REG, Math.round(w * B.SUBTITLE_SIZE), bc.body, textX, textY + Math.round(w * B.TITLE_SIZE) + Math.round(h * 0.04), { maxWidth: textW, lineHeight: 150 });
    ...
    for (var ii = 0; ii < items.length; ii++) {
        addBrochureFeatureItem(frame, ii + 1, textX, itemY + ii * itemGap, textW, items[ii].t, items[ii].d, bc, w);
    }
}
function buildBrochureSplitRight(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    ...
    var phoneY = laptopY + Math.round(laptopW * B.LAPTOP_RATIO) + Math.round(darkH * 0.04);
    addPhoneFrame(frame, "image-area-2", phoneW, phoneX, phoneY, bc);
...
function buildBrochureDualDevice(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    ...
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    addText(frame, "subtitle", "A brief tagline for this feature.", INTER_MED, Math.round(w * B.SUBTITLE_SIZE * 1.1), bc.subtext, margin, Math.round(h * 0.085), { maxWidth: contentW, align: "CENTER" });
    addText(frame, "description", "Detailed description of the feature and how it transforms operations. This gives context for the screenshots below.", INTER_REG, Math.round(w * B.BODY_SIZE), bc.body, Math.round(w * 0.12), Math.round(h * 0.13), { maxWidth: Math.round(w * 0.76), align: "CENTER", lineHeight: 160 });
...
// ══════════════════════════════════════════════════════════════
function buildBrochurePhoneBullets(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title — left aligned
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: Math.round(contentW * 0.5), letterSpacing: 3 });
    ...
    for (var li = 0; li < leftItems.length; li++) {
        addBrochureFeatureItem(frame, li + 1, margin, itemY + li * itemGap, Math.round(contentW * 0.42), leftItems[li].t, leftItems[li].d, bc, w);
    }
    ...
    addText(frame, "subtitle", "Visitor Management", resolveFont(BEBAS), Math.round(w * 0.03), bc.heading, Math.round(w * 0.48), Math.round(h * 0.42), { maxWidth: Math.round(contentW * 0.45), letterSpacing: 2 });
    ...
    for (var ri = 0; ri < rightItems.length; ri++) {
        addBrochureFeatureItem(frame, ri + 4, Math.round(w * 0.48), rightY + ri * Math.round(h * 0.08), Math.round(contentW * 0.45), rightItems[ri].t, rightItems[ri].d, bc, w);
    }
    // Dark block — bottom 30%
    var darkY = Math.round(h * 0.7);
    addBrochureBlock(frame, "dark-zone", w, h - darkY, 0, darkY, B.DARK_BG);
    // Title in dark zone
    addText(frame, "description", "Real-Time Visibility Unified", resolveFont(BEBAS), Math.round(w * 0.03), FIXED_WHITE, margin, darkY + Math.round(h * 0.03), { maxWidth: Math.round(contentW * 0.4), letterSpacing: 2 });
    ...
    for (var db = 0; db < darkBullets.length; db++) {
        addEllipse(frame, "bullet-dot", Math.round(w * 0.006), Math.round(w * 0.006), margin, darkY + Math.round(h * 0.08) + db * Math.round(h * 0.03), solid(bc.accent));
        addText(frame, "bullet-" + (db + 1), darkBullets[db], INTER_REG, Math.round(w * B.BODY_SIZE), FIXED_WHITE, margin + Math.round(w * 0.015), darkY + Math.round(h * 0.076) + db * Math.round(h * 0.03), { maxWidth: Math.round(contentW * 0.38), opacity: 0.85 });
    }
    // Laptop in dark zone — right side
    var darkLaptopW = Math.round(w * 0.48);
    addLaptopFrame(frame, "image-area-2", darkLaptopW, w - margin - darkLaptopW, darkY + Math.round(h * 0.04), bc);
}
function buildBrochureFeatureStack(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    ...
    addText(frame, "feature-1-title", "Feature One", resolveFont(BEBAS), Math.round(w * 0.032), bc.heading, margin, r1Y + Math.round(rowH * 0.12), { maxWidth: Math.round(contentW * 0.38), letterSpacing: 2 });
    addText(frame, "feature-1-desc", "A detailed description of this feature, explaining how it improves security operations and provides real-time visibility.", INTER_REG, Math.round(w * B.BODY_SIZE), bc.body, margin, r1Y + Math.round(rowH * 0.3), { maxWidth: Math.round(contentW * 0.38), lineHeight: 155 });
    addBrochureIconCircle(frame, Math.round(w * B.ICON_SIZE), margin, r1Y + Math.round(rowH * 0.62), bc, true);
    var laptopW1 = Math.round(contentW * 0.5);
    addLaptopFrame(frame, "image-area-1", laptopW1, w - margin - laptopW1, r1Y + Math.round(rowH * 0.08), bc);
    // Divider
    addRect(frame, "divider-1", contentW, dividerH, margin, r1Y + rowH, solid(bc.cardBorder));
    ...
    addPhoneFrame(frame, "image-area-2", phoneW2, margin + Math.round(contentW * 0.04), r2Y + Math.round(rowH * 0.06), bc);
    addText(frame, "feature-2-title", "Feature Two", resolveFont(BEBAS), Math.round(w * 0.032), bc.heading, Math.round(w * 0.38), r2Y + Math.round(rowH * 0.12), { maxWidth: Math.round(contentW * 0.5), letterSpacing: 2 });
    addText(frame, "feature-2-desc", "Explanation of how this feature streamlines visitor management and provides digital audit trails.", INTER_REG, Math.round(w * B.BODY_SIZE), bc.body, Math.round(w * 0.38), r2Y + Math.round(rowH * 0.3), { maxWidth: Math.round(contentW * 0.5), lineHeight: 155 });
    addBrochureIconCircle(frame, Math.round(w * B.ICON_SIZE), Math.round(w * 0.38), r2Y + Math.round(rowH * 0.62), bc, true);
    // Divider
    addRect(frame, "divider-2", contentW, dividerH, margin, r2Y + rowH, solid(bc.cardBorder));
    ...
    addText(frame, "feature-3-title", "Feature Three", resolveFont(BEBAS), Math.round(w * 0.032), FIXED_WHITE, margin, r3Y + Math.round(rowH * 0.12), { maxWidth: Math.round(contentW * 0.38), letterSpacing: 2 });
    addText(frame, "feature-3-desc", "How this capability provides unified real-time visibility across all sites and operations.", INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, margin, r3Y + Math.round(rowH * 0.3), { maxWidth: Math.round(contentW * 0.38), lineHeight: 155 });
    addBrochureIconCircle(frame, Math.round(w * B.ICON_SIZE), margin, r3Y + Math.round(rowH * 0.62), bc, true);
    var laptopW3 = Math.round(contentW * 0.42);
    addLaptopFrame(frame, "image-area-3", laptopW3, w - margin - laptopW3, r3Y + Math.round(rowH * 0.08), bc);
}
function buildBrochureScreenshotGallery(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title + subtitle
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    addText(frame, "subtitle", "See the platform in action across multiple views.", INTER_REG, Math.round(w * B.SUBTITLE_SIZE), bc.subtext, margin, Math.round(h * 0.085), { maxWidth: contentW, align: "CENTER" });
    ...
    addLaptopFrame(frame, "image-area-1", mainLaptopW, margin, darkY - Math.round(h * 0.02), bc);
    // Phone — top right, partially overlapping laptop
    var phoneW = Math.round(w * 0.18);
    var phoneX = margin + mainLaptopW - Math.round(phoneW * 0.1);
    addPhoneFrame(frame, "image-area-2", phoneW, phoneX, darkY + Math.round(h * 0.04), bc);
    ...
    addLaptopFrame(frame, "image-area-3", smallLaptopW, smallLaptopX, smallLaptopY, bc);
}
function buildBrochureStatsBanner(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title
    addText(frame, "title", "BY THE NUMBERS", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    // Red stats banner
    ...
    addRect(frame, "stats-banner", bannerW, bannerH, bannerX, bannerY, solid(bc.accent), bannerRadius);
    // 4 stats on banner
    ...
    for (var si = 0; si < stats.length; si++) {
        ...
        addText(frame, "stat-" + (si + 1) + "-number", stats[si].n, resolveFont(BEBAS), Math.round(bannerH * 0.38), FIXED_WHITE, statX, bannerY + Math.round(bannerH * 0.12), { maxWidth: statW, align: "CENTER" });
        addText(frame, "stat-" + (si + 1) + "-label", stats[si].l, INTER_REG, Math.round(bannerH * 0.18), FIXED_WHITE, statX, bannerY + Math.round(bannerH * 0.56), { maxWidth: statW, align: "CENTER", opacity: 0.75 });
        // Vertical divider between stats
        if (si < 3) {
            addRect(frame, "stat-divider-" + si, 1, Math.round(bannerH * 0.5), statX + statW, bannerY + Math.round(bannerH * 0.25), solid(FIXED_WHITE, 0.25));
        }
    }
    ...
    addBrochureFeatureItem(frame, 1, margin, calloutY, Math.round(contentW * 0.42), "Enterprise Grade", "Built for scale with multi-tenant architecture.", bc, w, { darkBg: true });
    addBrochureFeatureItem(frame, 2, margin + Math.round(contentW * 0.55), calloutY, Math.round(contentW * 0.42), "Compliant", "ISO 27001, SOC 2, and GDPR certified.", bc, w, { darkBg: true });
}
function buildBrochureCenteredShowcase(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    ...
    addRect(frame, "accent-line", lineW, 3, Math.round((w - lineW) / 2), Math.round(h * 0.04), solid(bc.accent));
    // Title centered
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE * 1.1), bc.heading, margin, Math.round(h * 0.06), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    // Description centered
    addText(frame, "description", "A comprehensive platform that brings together all your security and facility management operations into one unified dashboard.", INTER_REG, Math.round(w * B.BODY_SIZE * 1.1), bc.body, Math.round(w * 0.15), Math.round(h * 0.12), { maxWidth: Math.round(w * 0.7), align: "CENTER", lineHeight: 160 });
...
// ══════════════════════════════════════════════════════════════
function buildBrochureIconGrid2x3(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    ...
    addText(frame, "title", "EVERYTHING COVERED.", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), FIXED_WHITE, margin, Math.round(h * 0.04), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    addText(frame, "subtitle", "Everything your security operations need, built into one platform.", INTER_REG, Math.round(w * B.SUBTITLE_SIZE), B.LIGHT_GRAY, margin, Math.round(h * 0.095), { maxWidth: contentW, align: "CENTER" });
    // 2x3 card grid
    ...
    for (var ci = 0; ci < cardItems.length; ci++) {
        ...
        var card = addBrochureDarkCard(frame, "card-" + (ci + 1), cardW, cardH, cardX, cardY, cardRadius);
        var iconSize = Math.round(w * B.ICON_SIZE);
        var pad = Math.round(cardW * 0.08);
        addBrochureIconCircle(card, iconSize, pad, pad, bc, true);
        addText(card, "card-" + (ci + 1) + "-title", cardItems[ci].t, INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(cardH * 0.06), { maxWidth: cardW - pad * 2 });
        addText(card, "card-" + (ci + 1) + "-desc", cardItems[ci].d, INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
            iconSize +
            Math.round(cardH * 0.06) +
            Math.round(w * B.FEAT_TITLE) +
            Math.round(cardH * 0.06), { maxWidth: cardW - pad * 2, lineHeight: 150 });
    }
}
function buildBrochureIconGrid2x2(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Full dark background
    frame.fills = [solid(B.DARK_BG)];
    addText(frame, "title", "KEY CAPABILITIES", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), FIXED_WHITE, margin, Math.round(h * 0.04), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    addText(frame, "subtitle", "Four pillars of operational excellence.", INTER_REG, Math.round(w * B.SUBTITLE_SIZE), B.LIGHT_GRAY, margin, Math.round(h * 0.095), { maxWidth: contentW, align: "CENTER" });
    ...
    for (var ci = 0; ci < cardItems.length; ci++) {
        ...
        var pad = Math.round(cardW * 0.08);
        addBrochureIconCircle(card, iconSize, pad, pad, bc, true);
        addText(card, "card-" + (ci + 1) + "-title", cardItems[ci].t, INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(cardH * 0.04), { maxWidth: cardW - pad * 2 });
        addText(card, "card-" + (ci + 1) + "-desc", cardItems[ci].d, INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
            iconSize +
            Math.round(cardH * 0.04) +
            Math.round(w * B.FEAT_TITLE) +
            Math.round(cardH * 0.03), { maxWidth: cardW - pad * 2, lineHeight: 150 });
        // Screenshot placeholder inside card — bottom portion
        var phW = cardW - pad * 2;
        var phH = Math.round(cardH * 0.38);
        addBrochurePlaceholder(card, "card-" + (ci + 1) + "-image", phW, phH, pad, cardH - pad - phH, bc, Math.round(cardRadius * 0.6));
    }
}
function buildBrochureIconGrid3x2(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Full dark background
    frame.fills = [solid(B.DARK_BG)];
    addText(frame, "title", "PLATFORM OVERVIEW", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), FIXED_WHITE, margin, Math.round(h * 0.04), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    ...
    var wideCard1 = addBrochureDarkCard(frame, "card-1", wideW, rowH, margin, gridY, cardRadius);
    addBrochureIconCircle(wideCard1, iconSize, pad, pad, bc, true);
    addText(wideCard1, "card-1-title", "Primary Feature", INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(rowH * 0.04), { maxWidth: Math.round(wideW * 0.45) });
    addText(wideCard1, "card-1-desc", "Comprehensive overview of the main platform capability.", INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
        iconSize +
        Math.round(rowH * 0.04) +
        Math.round(w * B.FEAT_TITLE) +
        Math.round(rowH * 0.03), { maxWidth: Math.round(wideW * 0.45), lineHeight: 150 });
    // Image placeholder in right half of wide card
    var widePh = Math.round(wideW * 0.45);
    addBrochurePlaceholder(wideCard1, "card-1-image", widePh, rowH - pad * 2, wideW - pad - widePh, pad, bc, Math.round(cardRadius * 0.6));
    var narrowCard1 = addBrochureDarkCard(frame, "card-2", narrowW, rowH, margin + wideW + gapX, gridY, cardRadius);
    addBrochureIconCircle(narrowCard1, iconSize, pad, pad, bc, true);
    addText(narrowCard1, "card-2-title", "Feature Two", INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(rowH * 0.04), { maxWidth: narrowW - pad * 2 });
    addText(narrowCard1, "card-2-desc", "Description of this secondary capability.", INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
        iconSize +
        Math.round(rowH * 0.04) +
        Math.round(w * B.FEAT_TITLE) +
        Math.round(rowH * 0.03), { maxWidth: narrowW - pad * 2, lineHeight: 150 });
    // Row 2: Narrow card left + wide card right
    var row2Y = gridY + rowH + gapY;
    var narrowCard2 = addBrochureDarkCard(frame, "card-3", narrowW, rowH, margin, row2Y, cardRadius);
    addBrochureIconCircle(narrowCard2, iconSize, pad, pad, bc, true);
    addText(narrowCard2, "card-3-title", "Feature Three", INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(rowH * 0.04), { maxWidth: narrowW - pad * 2 });
    addText(narrowCard2, "card-3-desc", "Description of this additional capability.", INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
        iconSize +
        Math.round(rowH * 0.04) +
        Math.round(w * B.FEAT_TITLE) +
        Math.round(rowH * 0.03), { maxWidth: narrowW - pad * 2, lineHeight: 150 });
    var wideCard2 = addBrochureDarkCard(frame, "card-4", wideW, rowH, margin + narrowW + gapX, row2Y, cardRadius);
    addBrochureIconCircle(wideCard2, iconSize, pad, pad, bc, true);
    addText(wideCard2, "card-4-title", "Feature Four", INTER_SEMI, Math.round(w * B.FEAT_TITLE), FIXED_WHITE, pad, pad + iconSize + Math.round(rowH * 0.04), { maxWidth: Math.round(wideW * 0.45) });
    addText(wideCard2, "card-4-desc", "Description of this comprehensive platform feature.", INTER_REG, Math.round(w * B.BODY_SIZE), B.LIGHT_GRAY, pad, pad +
        iconSize +
        Math.round(rowH * 0.04) +
        Math.round(w * B.FEAT_TITLE) +
        Math.round(rowH * 0.03), { maxWidth: Math.round(wideW * 0.45), lineHeight: 150 });
    addBrochurePlaceholder(wideCard2, "card-4-image", widePh, rowH - pad * 2, wideW - pad - widePh, pad, bc, Math.round(cardRadius * 0.6));
}
// ══════════════════════════════════════════════════════════════
// BROCHURE LAYOUTS — Pattern 4: Special Layouts
// ══════════════════════════════════════════════════════════════
function buildBrochureComparison(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title
    addText(frame, "title", "WHY SWITCH?", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: contentW, align: "CENTER", letterSpacing: 3 });
    // Two columns
    ...
    // Vertical divider
    addRect(frame, "divider", 1, Math.round(h * 0.82), Math.round(w / 2), colY, solid(bc.cardBorder));
    // Subtle red tint on right column
    addRect(frame, "right-tint", colW + Math.round(w * 0.02), Math.round(h * 0.84), rightX - Math.round(w * 0.01), colY - Math.round(h * 0.01), solid(bc.accent, 0.03), Math.round(w * 0.008));
    // Left column — "The Old Way"
    var pillH = Math.round(h * 0.028);
    var pillW = Math.round(w * 0.14);
    addRect(frame, "old-pill", pillW, pillH, leftX, colY, solid(bc.cardBg), Math.round(pillH / 2));
    addText(frame, "col-1-subtitle", "The Old Way", INTER_SEMI, Math.round(pillH * 0.55), bc.subtext, leftX + Math.round(pillW * 0.12), colY + Math.round(pillH * 0.18));
    ...
    for (var bi = 0; bi < oldBullets.length; bi++) {
        addEllipse(frame, "old-dot-" + bi, Math.round(w * 0.007), Math.round(w * 0.007), leftX, bulletY + bi * bulletGap + Math.round(w * 0.003), solid(bc.subtext));
        addText(frame, "col-1-bullet-" + (bi + 1), oldBullets[bi], INTER_REG, Math.round(w * B.BODY_SIZE), bc.body, leftX + Math.round(w * 0.018), bulletY + bi * bulletGap, { maxWidth: colW - Math.round(w * 0.02) });
    }
    // Muted placeholder at bottom left
    addBrochurePlaceholder(frame, "col-1-image", Math.round(colW * 0.85), Math.round(h * 0.2), leftX, Math.round(h * 0.68), bc, Math.round(w * 0.008));
    // Right column — "The New Way"
    addRect(frame, "new-pill", pillW, pillH, rightX, colY, solid(bc.accent), Math.round(pillH / 2));
    addText(frame, "col-2-subtitle", "The New Way", INTER_SEMI, Math.round(pillH * 0.55), FIXED_WHITE, rightX + Math.round(pillW * 0.1), colY + Math.round(pillH * 0.18));
    var newBullets = [
    ...
    for (var ni = 0; ni < newBullets.length; ni++) {
        addEllipse(frame, "new-dot-" + ni, Math.round(w * 0.007), Math.round(w * 0.007), rightX, bulletY + ni * bulletGap + Math.round(w * 0.003), solid(bc.accent));
        addText(frame, "col-2-bullet-" + (ni + 1), newBullets[ni], INTER_SEMI, Math.round(w * B.BODY_SIZE), bc.heading, rightX + Math.round(w * 0.018), bulletY + ni * bulletGap, { maxWidth: colW - Math.round(w * 0.02) });
    }
    // Phone at bottom right
    var phoneW = Math.round(w * 0.17);
    addPhoneFrame(frame, "col-2-image", phoneW, rightX + Math.round((colW - phoneW) / 2), Math.round(h * 0.62), bc);
}
function buildBrochureTimeline(frame, w, h, bc) {
    var margin = Math.round(w * B.MARGIN);
    // Red accent bar
    addRect(frame, "accent-bar", w, Math.round(h * 0.005), 0, 0, solid(bc.accent));
    // Title
    addText(frame, "title", "HOW IT WORKS", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE), bc.heading, margin, Math.round(h * 0.03), { maxWidth: Math.round(w * 0.38), letterSpacing: 3 });
    // Vertical timeline line
    var lineX = margin + Math.round(w * 0.015);
    var lineTop = Math.round(h * 0.12);
    var lineBottom = Math.round(h * 0.85);
    addRect(frame, "timeline-line", 2, lineBottom - lineTop, lineX, lineTop, solid(bc.accent));
    ...
    for (var si = 0; si < steps.length; si++) {
        ...
        // Dot on the line
        addEllipse(frame, "step-dot-" + (si + 1), dotSize, dotSize, lineX - Math.round(dotSize / 2) + 1, stepY, solid(bc.accent));
        // Step text
        addText(frame, "step-" + (si + 1) + "-title", steps[si].t, INTER_SEMI, Math.round(w * B.FEAT_TITLE), bc.heading, textX, stepY - Math.round(dotSize * 0.2), { maxWidth: textW });
        addText(frame, "step-" + (si + 1) + "-desc", steps[si].d, INTER_REG, Math.round(w * B.BODY_SIZE), bc.body, textX, stepY + Math.round(w * B.FEAT_TITLE) + Math.round(h * 0.01), { maxWidth: textW, lineHeight: 150 });
    }
...
function buildBrochureFullBleed(frame, w, h, bc) {
    ...
    addEllipse(frame, "brand-circle", brandSize, brandSize, Math.round(w * B.MARGIN), Math.round(h * 0.025), solid(bc.accent));
    var margin = Math.round(w * B.MARGIN);
    var contentW = w - margin * 2;
    // Title
    addText(frame, "title", "FEATURE TITLE", resolveFont(BEBAS), Math.round(w * B.TITLE_SIZE * 1.2), FIXED_WHITE, margin, Math.round(h * 0.68), { maxWidth: contentW, letterSpacing: 3 });
    // Subtitle
    addText(frame, "subtitle", "One platform to command your entire operation.", INTER_MED, Math.round(w * B.SUBTITLE_SIZE * 1.2), FIXED_WHITE, margin, Math.round(h * 0.74), { maxWidth: Math.round(contentW * 0.7), opacity: 0.85 });
    ...
    for (var bi = 0; bi < bullets.length; bi++) {
        addEllipse(frame, "bullet-dot-" + bi, Math.round(w * 0.007), Math.round(w * 0.007), margin, bulletY + bi * bulletGap + Math.round(w * 0.003), solid(bc.accent));
        addText(frame, "bullet-" + (bi + 1), bullets[bi], INTER_REG, Math.round(w * B.BODY_SIZE), FIXED_WHITE, margin + Math.round(w * 0.018), bulletY + bi * bulletGap, { maxWidth: Math.round(contentW * 0.7), opacity: 0.8 });
    }
}
var BROCHURE_LAYOUTS = [
    {
        key: "hero-feature",
        suffix: "Hero Feature",
...
async function createBrochurePages(layouts, themes, resolution) {
    // Load fonts
    try {
        await Promise.all([
            figma.loadFontAsync(INTER_BOLD),
...
async function createBlogTemplates(blogTemplates, themes, bgPattern, bgDensity, title, author, sectionCount, logoImageBytes, fullLogoImageBytes) {
    // Load fonts
    try {
        await Promise.all([
            figma.loadFontAsync(INTER_BOLD),
...
figma.ui.onmessage = async (msg) => {
    ...
    if (msg.type === "create-templates") {
        await createTemplates(msg.platforms, msg.contentTypes, msg.brandFrame, msg.themes, msg.bgPattern || "none", msg.bgDensity || "medium", msg.qrImage, msg.logoImage, msg.fullLogoImage, msg.patternImage, msg.patternSvgStr);
        return;
    }
    if (msg.type === "create-blog") {
        await createBlogTemplates(msg.blogTemplates, msg.themes, msg.bgPattern || "none", msg.bgDensity || "medium", msg.title, msg.author, msg.sectionCount || 3, msg.logoImage, msg.fullLogoImage);
        return;
    }
    if (msg.type === "create-mockups") {
        await createMockups(msg.layouts || [], msg.deviceColor || "space-black", msg.themes || ["dark"], msg.platforms || [], msg.postDeviceFocus || ["phone"], msg.desktopStyle || "macbook", msg.logoImage, msg.fullLogoImage);
        return;
    }
    if (msg.type === "create-brochure") {
        await createBrochurePages(msg.layouts || [], msg.themes || ["light"], msg.resolution || "digital");
        return;
    }
    ...
    const templates = selection.filter((n) => n.type === "FRAME" || n.type === "COMPONENT" || n.type === "INSTANCE");
    ...
    await Promise.all(Array.from(uniqueFonts.values()).map((f) => figma.loadFontAsync(f)));
    ...
    for (const template of sortedTemplates) {
        ...
        for (let i = 0; i < variations.length; i++) {
            ...
            for (const [key, value] of Object.entries(variation)) {
                ...
                const textNode = findTextByName(clone, key);
                if (textNode) {
            ...
            if (qrUrl && msg.qrImages && msg.qrImages[qrUrl]) {
                const qrRect = clone.findOne((n) => n.name === "qr-code" && n.type === "RECTANGLE");
                if (qrRect) {
